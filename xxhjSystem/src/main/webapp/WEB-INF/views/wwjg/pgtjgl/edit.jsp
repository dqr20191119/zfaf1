...
<div>
	<cui:form id="formId_pgtjgl_edit">
		...
		<cui:input name="scflg" value="1" type="hidden"/>
 		...
 		<table class="table">
			...
			<tr>
				<th>评估条件：</th>
				<td>
					<cui:textarea height="200" name="name" required="true" width="250"></cui:textarea>
				</td>				
			</tr>
			<tr>
				<th>扣分值：</th>
				<td>
					<cui:input name="kfz"  id="kfz" required="true" width="250"/>
				</td>				
			</tr>
			<tr>
         	<th>备注：</th>
...
<script>
	...
	$.parseDone(function() {
 		...
 		var id='${id}';
		if(id) {
			loadForm("formId_pgtjgl_edit", "${ctx}/wwjg/pgtjgl/getById?id=" + id, function(data) {
  ...
  function huoquFxdjmc(){
	...
	function getCodes(){
		var code = $("#code").textbox("getValue");
		 $.ajax({
				type: 'post',
				url: '${ctx}/wwjg/pgtjgl/getByCode?code='+code,
				dataType: 'json',
				success: function(data) {
					if(data=="1"){
						$.alert("此编码已存在！");
