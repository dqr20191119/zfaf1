...
$.component("coral.tree",{
	castProperties : ['data','fontCss', 'rootNode', 'chkboxType','formatter','showIcon','addDiyDom','beforeClick','beforeDrag','onDrag','onDrop'],
	version: "4.0.2",
	componentName: "tree",
...
//		this.compContent.append(obj);
	...
	filterNodesByParam:  function (keysObj) {
		...
		if ( typeof keysObj === "object" ) {
			$.each( keysObj, function (k, v) {
				...
				engine = that._pinyinSetCache (engine, k, nodesArr);	
				//nodesFilter = nodesFilter.concat( that.getNodesByParamFuzzy (k, v) );
				//if (k == keyObj) {
					that._pinyinSearch(engine, v, 
							function (dataFilter) {
								that.showNodes (dataFilter, {showParents: true});	
						});
				//}
	...
	_onTreeContextmenu: function(that, node, e) {
		var opts = that._getSetting(that.options.treeId);
		var event = e || window.event;
		if (that._apply(opts.beforeRightClick, [opts.treeId, node, opts], true)) {
			that._trigger("onRightClick", event, [opts.treeId, node, opts]);
		}
	...
	_makeUlHtml: function(opts, node, html, content) {
		html.push("<ul id='", node.tId, _final.id.UL, "' class='", _final.className.LEVEL, node.level, " ", this._makeUlLineClass(opts, node), "' style='display:", (node.open ? opts.nodeUrlClass: "none"),"'>");
		html.push(content);
		html.push("</ul>");
	},
