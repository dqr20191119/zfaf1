...
$.component( "coral.grid", $.coral.grid, {
	...
	//	that.options.selrow = that.rows[iRow].id;
		...
		if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
			if(parseInt(that.options.iCol,10)>=0  && parseInt(that.options.iRow,10)>=0) {
				$("td:eq("+that.options.iCol+")",that.rows[that.options.iRow]).removeClass("edit-cell");
		...
		//	var coralOption = {"onValidSuccess":_onValidSuccess,"onValidError":_onValidError,"onKeyDown":_keydown, "value":$(elc).val()};
			...
			switch ( cm.edittype ) {
				...
				case "textarea":
					...
					type = (type =="text" || type =="textarea")? "textbox" : type;
					$(elc)[type](editoptions);					
					$( elc )[type]( "setValues", $(elc).val().split(",") );
					break;					
				case "datepicker":
					editoptions = $.extend({}, editoptions, coralOption);
	...
	GridNav : function() {
		...
		var selection = $("<div style='position:fixed;top:0px;width:1px;height:1px;' tabindex='0'><span tabindex='-1' style='width:0px;height:0px;background-color:grey' id='"+that.options.knv+"'></span></div>"),
		i, kdir;
