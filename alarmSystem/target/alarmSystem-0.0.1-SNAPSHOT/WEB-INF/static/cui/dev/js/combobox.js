...
$.component( "coral.combobox", $.coral.combo, {
		...
		for (i = 0; i < data.length; i++) {
			...
			if( value.length !== 0){
				if($.inArray(v,value) !== -1){
					classes += " coral-combobox-item-selected";
				}
			}
			if (formatterEvent) {
	...
	_doQuery : function(q) {
		...
		if (opts.mode == "remote") {
			...
			item.hide();
			var hasResult = false;     //body设置overflow:hidden，下拉面板overflow:auto，搜索无数据时。CORALIV-1687
			//重新查询前，将之前的高亮标签删除
			...
			//处理IE9下coral-combo-content高度不正确。
			if (!hasResult) {
				panel.css("height", "1px");
			} else {
...
//				that._trigger($.isFunction( opts.onLoad ) ? opts.onLoad:"onLoad", null, [data]);
	...
	select: function(value) {
		...
		//data[matchedIndex].selected = true;
	},
	...
	unselect: function(value) {
		...
		for(i = 0; i < values.length; i++) {
			if (values[i] == value) {
				...
				//data[matchedIndex].selected = false;
				break;
			}
		}
	},
