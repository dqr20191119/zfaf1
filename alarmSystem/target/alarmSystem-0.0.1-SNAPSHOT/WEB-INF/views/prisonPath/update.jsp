				...
				<cui:form id="formId_updateInfo" heightStyle="fill">
					<table class="table" style="width: 98%">
						...
						<tr>
							<th>路线类别：</th>
							<td>
								<cui:combobox id="ppiPathType" name="ppiPathType" data="pathType" componentCls="form-control"
...
<script>
	$.parseDone(function() {
		//部门数据请求
		$("#ppiDprtmntId").combobox( "reload", "${ctx}/common/authsystem/findAllChildrenOrgByJyKeyForCombobox.json?cusNumber=" + cusNumber);
		...
		$.ajax({
			type : "post",
			url : "${ctx}/prisonPath/findById.json?id=${ID}",
			dataType : "json",
		...
		var url = "${ctx}/prisonPath/listAllForPrisonPathCamera?pcrPathId=${ID}";
		$("#gridPath").grid("reload", url);
	...
	function update() {
		if ($("#formId_updateInfo").form("valid")) {
			...
			var path = $("#formId_updateInfo").form("formData");
			...
			$.ajax({
				type : 'post',
				url : '${ctx}/prisonPath/updateInfo.json',
	...
	function cancelRltn() {
		...
		var ids = [];
		for (var i = selected.length; i > 0; i--) {
			var patnCameraDate = $("#gridPath").grid("getRowData", selected[i - 1]);
	...
	function removeCamera(data_) {
		if (data_.length > 0) {
			$.confirm("是否删除选中的记录", "信息确认", function(confirm) {
				if (confirm) {
					$.ajax({
						url : "${ctx}/prisonPath/delete/camera.json",
						dataType : "json",
						type : "post",
