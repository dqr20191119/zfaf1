...
<script>
	...
	//	$("#combId_area").combotree("tree").tree("reload", "${ctx}/common/areadevice/findForCombotree.json?cusNumber=" + cusNumber + "&deviceType=0");
		...
		setTimeout(function () {
			var url = "${ctx}/broadcastPlan/queryById.json?id=${id}";
			$.ajax({
				type : "post",
				url : url,
				dataType : "json",
				success : function(data) {
					if (data.success) {
						info = data.obj;
						console.log("广播预案参数 info = " + JSON.stringify(info));
						$("#formId_gb_update").form("load", info);
						//
						$("#gridRightData").grid("reload",info.broadcasts);
					} else {
						$.messageQueue({
							message : data.msg,
							cls : "warning",
	...
	function update() {
		...
		//	formData['bbdArea'] = $("#combId_area").combotree("getText");
         //   formData["bbdCameraName"] = $("#comboId_glsxt").autocomplete("getText");
			...
			var url = '${ctx}/broadcastPlan/update.json?id=${id}';
			$.ajax({
				type : 'post',
				url : url,
				data : formData,
