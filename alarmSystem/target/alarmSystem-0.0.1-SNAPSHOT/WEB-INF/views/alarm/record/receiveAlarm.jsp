...
<cui:input id="flag" componentCls="form-control_hide" value="0" type="hidden"></cui:input>
<!-- 0代表没有接警人，1代表有接警人 -->
<cui:input id="flowId" componentCls="form-control_hide" type="hidden"></cui:input>
...
<div class="oprtrDiv">
	...
	<div class="alarm_button" align="center">
		<cui:button id="back_1" onClick="back(1)" label="返回"></cui:button>
...
<script>
	...
	function init(alarmMsg) {
		...
		if (oprtnStts == "3") {//处置状态 3 完成
			...
			$("#subButton").hide();
			$("#leaderHandle").hide();
			$("#hfdaXcqk").textbox("option", "readonly", true);
			$("#hfdaCzqk").textbox("option", "readonly", true);
		}
       ...
       if(hfdaXcqk !=null && hfdaXcqk !="" && hfdaXcqk != undefined){
        ...
        $.ajax({
            ...
            success : function(data) {
                if (data.code ==200 ) {
                   //清除记录表的详情id 的值
                    //根据当前步骤  隐藏$(".receiveDiv").hide(); 显示下一步的填写记录按
                    if(type !=2){
                        $(".receiveDiv").hide();
                        $(".oprtrDiv").show();
