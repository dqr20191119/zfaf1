...
$.component( "coral.combotree", $.coral.combo, {
	version: "4.0.3",
	castProperties : ["data", "rootNode","buttons","shortCut","fontCss","beforeClick"],
	options: {
		valueField: 'id',
		textField : 'name',
	...
	_create : function() {
		...
		if ( this.options.popupDialog ) {
			...
			this.uiCombo.popupDialog.dialog({
				...
				height : this.options.dialogHeight == "auto"? "auto": parseInt(this.options.dialogHeight),
				width : this.options.dialogWidth == "auto"? "auto": parseInt(this.options.dialogWidth),
				position: this.options.dialogPosition,
				modal : true,
				resizable: false,
				buttons: {
					"确定" : function(e) {
	...
	_getSetting: function(){
		...
		var setting = {
			...
			nodeUrlClass:this.options.nodeUrlClass,
			...
			onClick : function( e, treeId, treeNode, clickFlag ) {
				var textArr = [];
				var valueArr = [];
				textArr.push( treeNode[that.options.textField] );
				valueArr.push( treeNode[that.options.valueField] );
			...
			onCheck : function(e, treeId, treeNode){
				...
				for (var i=0, l=nodes.length; i<l; i++) {
					...
					if(that.options.cascadeCheck || !nodes[i].getCheckStatus().half){
						...
						textArr.push(nodes[i][that.options.textField]);
						valueArr.push(nodes[i][that.options.valueField]);
					}
				}
	...
	_initData : function() {
		...
		if (this.options.simpleDataEnable) {
			this.options.valueField = this.options.simpleDataIdKey? this.options.simpleDataIdKey: this.options.valueField;
			this.options.textField = this.options.keyName? this.options.keyName: this.options.textField;
		}
		//初始化tree
	...
	_setTreeNodeState: function(values, remainText) {
		...
		if ($cbTree.tree("option", "checkable") == true) {
			...
			// 设置checkbox
			for(i in values){
				var node = $cbTree.tree("getNodeByParam",this.options.valueField, values[i]);
				if (node) {
				...
				if ( values || !remainText) {
					if (node) {
						nodeName.push(node[this.options.textField]);
					} else {
