...
//			var button = $("<input type='button'/>").appendTo(warp);
		...
		this.originLimit = this.options.filesLimt;
	},
	...
	cancel: function(fileID, supressEvent) {
		...
		if(!settings.disabled){
			//flash上传成功之后删除，filesLimt需要动态加，否则删除之后再传，仍然会超出限制
			var filesLimit = Number(this.options.filesLimt),
				stats = this.dealLimit;    //全局变量
			if (typeof fileID === "array" || typeof fileID === "undefined") {
				...
				if (typeof fileID === "undefined") {
					...
					filesLimit = filesLimit + this.originLimit;
				} else {
					...
					filesLimit = filesLimit + args.length;
				}
			} else {
				var item = $('#' + fileID);
				$item = $(item);
			...
			//files_queued表示队列里已经全部上传，没有待上传的文件；
			//successful_uploads上传成功的文件数
			if ( stats && stats.files_queued === 0 && stats.successful_uploads === Number(this.options.filesLimt)) {
				this._setOption("filesLimt", filesLimit, true);
			}
		}
	},
	...
	onSelect: function(swfObj, file) {
		...
		if ($.inArray('onSelect', settings.overrideEvents) < 0) {
			...
			//$('#' + settings.queueID).prepend(item);
			$('#' + settings.queueID)[settings.prependFiles ? 'prepend' : 'append'](item);
