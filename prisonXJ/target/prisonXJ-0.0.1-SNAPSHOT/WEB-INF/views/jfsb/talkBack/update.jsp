...
<div style="text-align: center; height: 100%; width: 100%">
	<cui:form id="formId_djzj_update" heightStyle="fill">
			<table class="table" style="width: 98%">
			<tr>
				<th>编号：</th>
				<td>
					<cui:input name="tseIdnty" componentCls="form-control" required="true"></cui:input>
				</td>
				<th>IP：</th>
				<td>
					<cui:input validType="ip" name="tseIp" componentCls="form-control" required="true"></cui:input>
				</td>
			</tr>
			<tr>
				<th>名称：</th>
...
<script>
	...
	$.parseDone(function() {
        ...
        var url = "${ctx}/talkBackServer/findById.json?id=${ID}";
        $.ajax({
            type : "post",
            url : url,
            dataType : "json",
            success : function(data) {
                if (data.success) {
                    info = data.obj;
                    $("#formId_djzj_update").form("load", info);
                    setTimeout(function(){
                        $("#areaTreeId").combotree("setText",info.tseArea);
                    });
                } else {
	...
	//发送修改请求
	function update() {
		if ($("#formId_djzj_update").form("valid")) {
			var formData = $("#formId_djzj_update").form("formData");
			...
			var url = '${ctx}/talkBackServer/update/info.json?id=${ID}';
			$.ajax({
				type : 'post',
				url : url,
				data : formData,
