...
/**
 * 单点登录令牌工具类,提供给第三方系统使用，用于解析框架中生成的令牌，第三方系统调用方法 String[] tokenArray = TokenUtils.decodeTokenFromUrl(String token),tokenArray[0]=用户名 ,tokenArray[1]= 令牌生成时间.
 * <p>Company:上海中信信息发展股份有限公司</p>
 * @date 2013-11-11 上午10:26:59
 * @version 1.0.2013-11-11
 */
public class TokenDUtils {
	...
	public static void main(String[] args) throws Exception{
		//MzIwMTA2MTk2MjA4MDExMjIzfOWNl+S6rOWls+WtkOebkeeLsToxNDM4MzIxMDQ5MzAzOjViNzc1MWM4YmNmZDExZTQzZWVjZTE5NTczNmMyMWM0
		String token = "MzIzNjAwNXzEz76pxa7X07zg0/w6MTQzODQxMzk1NDU0OTo4MTc4Nzg2NWNjNjRkYzQ1MTI4ZWJiN2MzYjNmMmE5OA";
		//String[] tokenArray = TokenUtils.decodeToken(token);
		String[] tokenArray = TokenDUtils.decodeTokenFromUrl(token);
		if(tokenArray != null && tokenArray.length>0)
		for(String tok : tokenArray)
			System.out.println(tok);

//		Date date = new Date(1434092175553l);
...
//		System.out.println(format.format(date));
	...
	public static String[] decodeTokenFromUrl(String token) throws Exception{
		...
		try {
			System.out.println("解析令牌：" + token);
			tokenArray = TokenDUtils.decodeToken(token);
			System.out.println("解析结果：" + JSON.toJSONString(tokenArray));
			if (tokenArray.length != 3) {
//	            throw new Exception("Cookie token did not contain 3 tokens");
	            throw new Exception("登录令牌解析错误!");
	        }
...
//	            throw new Exception("Cookie token[1] did not contain a valid number (contained '" +tokenArray[1] + "')");
	        	throw new Exception("登录令牌解析错误!");
	        }
	        if (isTokenExpired(tokenExpiryTime)) {
//	            throw new Exception("Cookie token[1] has expired (expired on '" + new Date(tokenExpiryTime) + "'; current time is '" + new Date() + "')");
	            throw new Exception("登录令牌已失效!");
	        }
	        ...
	        if (!equals(expectedTokenSignature,tokenArray[2])) {
	            throw new Exception("Cookie token[2] contained signature '" + tokenArray[2] + "' but expected '" + expectedTokenSignature + "'");
	        }*/

		} catch (IOException e) {
//			throw new Exception("the token from url is error'");
			throw new Exception("登录令牌解析错误!");
		}
		return tokenArray;
	}
	...
	//============================================================================================================
	//检查令牌私有方法 开始
	private static String[] decodeToken(String cookieValue) throws IOException{
        ...
        //new
        //String cookieAsPlainText = new String(Base64.decode(cookieValue.getBytes()));//该版本没有指定转码规则，如果是相同框架系统进行生成和解析令牌，不存在编码问题，如果生成的令牌提供给其他系统使用，则其他系统在解析令牌时可能存在编码错误，导致乱码
        String cookieAsPlainText = new String(Base64.decode(cookieValue.getBytes(encode)),encode);
        String[] tokens = delimitedListToStringArray(cookieAsPlainText, DELIMITER);
...
class InvalidBase64CharacterException extends IllegalArgumentException {
	private static final long serialVersionUID = 5287762210109069105L;
