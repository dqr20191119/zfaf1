...
<script>
	...
	$.parseDone(function() {
		...
		/*$("#cdmOrderCount").textbox({
			readonly:true
		});*/
		if(USER_LEVEL =='3'){
            $("#cdmOrderCount").textbox("setValue",3);
        }else{
            $("#cdmOrderCount").textbox("setValue",4);
        }
			...
			if(orderCount){
				...
				for(var i = 0; i < orderCount; i++){ 
					...
					item=("<fieldset><legend>班次信息</legend>"+
							...
							"<td width='150'align='right'>值班时间：&nbsp;&nbsp;&nbsp;</td><td><input class='coralui-textbox' id='orderStartTime_"+(i+1)+"' data-options='\"width\":\"150\"'/></td>"+
							"<td>~</td><td><input class='coralui-textbox' id='orderEndTime_"+(i+1)+"' data-options='\"width\":\"150\"' /></td>"+
							"<td width='150'align='right'>岗位数量：&nbsp;&nbsp;&nbsp;</td><td><input class='coralui-textbox' id='positionCount_"+(i+1)+"' name='positionCount_"+(i+1)+"' value="+jobCount[i]+"    data-options='\"onChange\": \"addPosition(" + (i+1) + ")\", \"width\":\"150\",\"validType\":\"naturalnumber\",\"required\": true'' /></td></tr></table></fieldset> ");
						
					tab.append("<tr><td>" +item+ "</td></tr>");
				//	debugger;
					//var jobCountId ="#positionCount_"+(i+1);
					//$(jobCountId).val(jobCount[i]);
					//$(jobCountId).text(jobCount[i]);
					//$(jobCountId).textbox("setValue",jobCount[i]);
					//console.log($(jobCountId).val())
					$.parser.parse(tab);
