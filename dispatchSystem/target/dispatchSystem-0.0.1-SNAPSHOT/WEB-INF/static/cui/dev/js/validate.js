...
$.validate = {
	...
	_validate: function (json) {
		...
		switch (type) {
			...
			case "charLeft":
				return this._checkCharLeft(validValue, elementValue, errMsg,field,eventType);
			default:
				return "";
		}			
	},
	...
	validTypeNames: ["required","minlength","maxlength","pattern","validType","valid","specialCharacters","charLeft"],
	// 校验类型控制 ??
	validTypeOptions: {
		"maxlength": {
			restrictInput: false,
	...
	validateField: function( event, data ) {
		...
		var el = data.element,
			...
			eventType = event!=null ? event.type : null,
	...
	_checkValid: function (validValue, elementValue, errMsg, field) {
		...
		if (validValue) {
			validResult = $.coralApply ($.coral.toFunction(validValue),field&&field[0], [{"value": elementValue,"ele":field}]);
...
//			},
	...
	_checkCharLeft:function(validValue, elementValue, errMsg,field,eventType){
		var json = {
				color : "#999",
				position : "absolute",
				background:"#E4EAF5",
		...
		var label = "<div class='coral-validate-charLeft tooltip'>还可以输入"+validValue+"个字符</div>";
		if($(field).parent().find(".coral-validate-charLeft").length == 0){
			$(field).after(label);
		}
		var top = $(field).parent().position().top-22;
		$(".coral-validate-charLeft",$(field).parent()).css(json).css({left : $(field).parent().position().left + "px", top : top+ 'px'}); 
		if (validValue && this._isPositiveInteger(validValue)) {
			var maxLength = parseInt(validValue), nowLength = this._getByteLength(elementValue);
			//需求要求提示信息不能和错误信息一起出现
			var len = maxLength - nowLength,
				show="还可以输入"+len+"个字符", 
				id = $(field).attr("id");
			var idOrName = (id==null||id=="")?$(this).attr("name"):id;
			var errdiv = $(".coral-validate-state-error",$(field).parent().parent());
			if (len >= 0 ) {
				if (errdiv.length == 0 && eventType!= "textboxonchange" && eventType != "textboxonblur" && eventType != null) {
					$(".coral-validate-charLeft",$(field).parent()).html(show).show();
				} else {
					$(".coral-validate-charLeft",$(field).parent()).remove();
				}
			} else {
				var errMsgResult = "已超出文字限制范围";
