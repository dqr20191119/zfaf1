...
$.component ( "coral.textbox", $.coral.inputbase, {
	...
	_initUpload: function(){
		var that = this,
		//许多小部件需要元素生成唯一的 id。.uniqueId() 会检查元素是否有 id，如果元素没有 id，它将生成一个 id，并设置为该元素的 id。
			id = that.element.uniqueId().attr("id");
		that.select = [];
	...
	_updateLabel: function(labelField) {
		this.labelPanel.html(labelField);
	},
	//组件初始化
	_initElement: function () {
		...
		if (this.element[0].getAttribute("value")) {
	...
	_bindEvents: function () {
		...
		coral.eventsQueue.push({instance: this, args: [{
			...
			"blur": function( event ) {
				...
				if(this.component().find(".coral-validate-charLeft").length != 0){
					$(".coral-validate-charLeft").hide();
				}
				if ( "" === this.element.val() ) {
					this._showPlaceholder();
				}
			...
			"keydown": function( event ) {
				...
				return $.validate.restrictInput(this.element, event);
				return r;
			},
