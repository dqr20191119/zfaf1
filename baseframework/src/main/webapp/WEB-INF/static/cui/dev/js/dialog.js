...
$.component("coral.dialog", {
	...
	options: {
		...
		width: 300,
		url: "",
		autoBlur: false,  //关闭dialog后，打开dialog的按钮是否聚焦。默认聚焦
		isFocusFirst: false,  //点击dialog，并且dialog位于页面最前时是否需要聚焦第一个表单元素，默认不聚焦
		contentType: "",
		reLoadOnOpen: false,
		postData: [],
...
//				},800)
	...
	open: function() {
		...
		this._show (this.uiDialog, this.options.show, function() {
			//that.uiDialog.position(that.options.position);
			if ($.inArray(that.options.wtype, ["dialog","alert", "confirm"]) > -1) {
				that._focusTabbable();
			}
			if (!canRequest) {
	...
	_createWrapper: function() {
		...
		//message位置有问题
		/*if(this.options.appendTo == "parent" || $(this.options.appendTo)[0].tagName.toLowerCase() != "body"){
		...
		this._on(this.uiDialog, {
			...
			mousedown: function (event) {
				if ( this._moveToTop(event) && this.options.isFocusFirst) {
					this._focusTabbable();
				}
			}
		});
		...
		if (this.options.minimizable) {
			...
			this.uiDialogTitlebarMaximum = $("<button type='button'></button>")
				.button({
					...
					text: false,
					countdown: false   //button在延时的情况下,如果dialog重新初始化,延时调用的方法会报错
				})
