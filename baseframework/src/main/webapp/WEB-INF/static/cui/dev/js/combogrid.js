...
$.component( "coral.combogrid", $.coral.combo, {
	...
	options: {
		...
		loader: function(param, success, error) {
			var that = this,
			    instance = $(this),
			    url = instance.combogrid("option", "url");
			if (!url) return false;
			if(param.q){
				param = {q:param.q.join(",")};
			}
			$.ajax({
				type: instance.combogrid("option", "method"),
				url:  url,
				data: param,
				dataType: "json",
	...
	reload: function( url,param ) {
		...
		if ( isUrl ==true ) {
			...
			if($.isEmptyObject(param)){
				...
				url = url.indexOf("?") != -1? url + "&q=" + param.q: url + "?q=" + param.q;
				$grid.grid("reload", url);
				/*loaderEvent.apply(this.element, [param, function(data) {
					//url = data;
					$grid.grid("reload", data);
				}, function() {
