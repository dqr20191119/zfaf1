...
import com.cesgroup.prison.httpclient.zfxx.dto.ZfShgxDto;
...
public class ZfShgxHttpClient {
	...
	private static ZfShgxHttpClient currentWSHttpClient = new ZfShgxHttpClient();
	
	private ZfShgxHttpClient() {
		
	}
	
	private ZfShgxHttpClient(String host) {
		ZfShgxHttpClient.host = host;
	}
	
	public static synchronized ZfShgxHttpClient getWSHttpClient(String host) {
		ZfShgxHttpClient.host = host;
		return currentWSHttpClient;
	}
	
	public static synchronized ZfShgxHttpClient getWSHttpClient() {
		if(currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfShgxHttpClient();
	}
	...
	public static List<ZfShgxDto> entityZfShgx(String time, String corp, String scorp, String dept) throws RESTHttpClientException {
		// 罪犯社会关系信息DTO
		List<ZfShgxDto> zfShgxDtoList = new ArrayList<ZfShgxDto>();
		try {
			if(time != null && corp != null) {
				// Description: 请求Url拼接参数
				StringBuffer requestUrl = new StringBuffer("/zfxx/entity/entity_zf_shgx");
							...
							if(defaultZfJbxxArray != null && defaultZfJbxxArray.size() > 0) {
								...
								for(JsonElement defaultZfJbxxElement : defaultZfJbxxArray) {
									...
									if(zfJbxxDto != null && Util.notNull(zfJbxxDto.getCId())) {
										...
										// 罪犯社会关系信息Json数组
										JsonArray defaultZfShgxArray = Util.notNull(defaultZfJbxxObject.get("Default_ZfShgx")) ? defaultZfJbxxObject.getAsJsonArray("Default_ZfShgx") : null;
										
										// 罪犯社会关系信息Json数组转化为罪犯社会关系DtoList
										List<ZfShgxDto> zfShgxDtoTempList = ZfShgxHttpClient.zfShgxJsonArrayToZfShgxDtoList(zfJbxxDto, defaultZfShgxArray);
										
										// 罪犯社会关系信息Dto list存值
										zfShgxDtoList.addAll(zfShgxDtoTempList);
									}
								}
							}
						}
			...
			throw new RESTHttpClientException("调用ZfShgxHttpClient获取罪犯社会关系信息发生异常, Exception info is " + e);
		}
		return zfShgxDtoList;
	}

	/**
	 * 罪犯社会关系信息Json数组转化为罪犯社会关系信息DtoList
	 * 
	 * @param defaultZfLbcArray
	 * @return
	 */
	private static List<ZfShgxDto> zfShgxJsonArrayToZfShgxDtoList(ZfJbxxDto zfJbxxDto, JsonArray defaultZfShgxArray) {
		List<ZfShgxDto> zfShgxDtoList = new ArrayList<ZfShgxDto>();
		if(zfJbxxDto != null && defaultZfShgxArray != null && defaultZfShgxArray.size() > 0) {
			for(JsonElement defaultZfShgxElement : defaultZfShgxArray) {
				ZfShgxDto zfShgxDto = Util.fromJson(defaultZfShgxElement, ZfShgxDto.class);
				if(zfShgxDto != null) {
					zfShgxDto.setCZfbh(zfJbxxDto.getCZfbh());
					zfShgxDto.setCId(zfJbxxDto.getCId());
					zfShgxDto.setDUrlTime(zfJbxxDto.getDUrlTime());
					zfShgxDtoList.add(zfShgxDto);
				}
			}
		}
		return zfShgxDtoList;
