package com.cesgroup.prison.httpclient.moodRecord;
...
public class MoodRecordClient {
     ...
     * 日志工具
     */
	private static final Logger logger = LoggerFactory.getLogger(MoodRecordClient.class);
	/**
	 * Gson工具
	 */
	private final static Gson gson = new GsonBuilder().create();

	private static MoodRecordClient currentWSHttpClient = new MoodRecordClient();

	private MoodRecordClient() {
	}
	
	private MoodRecordClient(String host) {
		MoodRecordClient.host = host;
	}

	public static synchronized MoodRecordClient getWSHttpClient(String host) {
		MoodRecordClient.host = host;
		return currentWSHttpClient;
	}

	public static synchronized MoodRecordClient getWSHttpClient() {
		if (currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new MoodRecordClient();
	}
	 ...
	 */
	public static PageInfo<MoodRecordDto> entityQjjcjl(String time, String corp,PageRequest pageRequest, String sTime, String eTime, String statues, String zfName, String zfMood) throws RESTHttpClientException {
		PageInfo<MoodRecordDto> MoodRecordDtoPage = new PageInfo<MoodRecordDto>();
		int pageNum = 0,pageSize=0,pages=0,total=0;
		List<MoodRecordDto> qjjcjlDtoList = null;
		try {
			if(time != null && corp != null) {
				// Description: 请求Url拼接参数
				StringBuffer requestUrl = new StringBuffer("/infoexc/clearcheck.do?action=getClearcheckList2");
				
				// 查询条件
				JsonObject filter = new JsonObject();
				filter.addProperty("name", zfName);
				filter.addProperty("mood", zfMood);
				filter.addProperty("spstatus", statues);
					...
					if("数据获取成功".equals(returnCode)) {
						...
						if(returnData != null) {
							...
							if(defaultQjjcjlArray != null && defaultQjjcjlArray.size() > 0) {
								...
								qjjcjlDtoList = new ArrayList<MoodRecordDto>();
								for(JsonElement defaultQjjcElement : defaultQjjcjlArray) {
									...
									MoodRecordDto MoodRecordDto = Util.fromJson(defaultZfJbxxObject, MoodRecordDto.class);
									if(MoodRecordDto.getStatus().equals("H")) {
										MoodRecordDto.setStatus("开心快乐");
									}else if(MoodRecordDto.getStatus().equals("N")) {
										MoodRecordDto.setStatus("正常平和");
									}else if(MoodRecordDto.getStatus().equals("S")) {
										MoodRecordDto.setStatus("伤心低落");
									};
									if(MoodRecordDto.getSpstatus().equals("0")) {
										MoodRecordDto.setSpstatus("待处理");
									}else if(MoodRecordDto.getSpstatus().equals("1")) {
										MoodRecordDto.setSpstatus("已处理");
									};
									if(MoodRecordDto != null && Util.notNull(MoodRecordDto.getId())) {
										qjjcjlDtoList.add(MoodRecordDto);
									}
								}
							}
						}
		...
		MoodRecordDtoPage.setList(qjjcjlDtoList);
		MoodRecordDtoPage.setPageNum(pageNum);
		MoodRecordDtoPage.setPages(pages);
		MoodRecordDtoPage.setPageSize(pageSize);
		MoodRecordDtoPage.setTotal(total);
		return MoodRecordDtoPage;
	}
