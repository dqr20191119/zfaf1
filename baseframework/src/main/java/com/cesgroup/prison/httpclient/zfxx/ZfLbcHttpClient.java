...
public class ZfLbcHttpClient {
	...
	private static ZfLbcHttpClient currentWSHttpClient = new ZfLbcHttpClient();
	
	private ZfLbcHttpClient() {
		
	}
	
	private ZfLbcHttpClient(String host) {
		ZfLbcHttpClient.host = host;
	}
	
	public static synchronized ZfLbcHttpClient getWSHttpClient(String host) {
		ZfLbcHttpClient.host = host;
		return currentWSHttpClient;
	}
	
	public static synchronized ZfLbcHttpClient getWSHttpClient() {
		if(currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfLbcHttpClient();
	}
	...
	public static List<ZfLbcDto> entityZfLbc(String time, String corp, String scorp, String dept) throws RESTHttpClientException {
		// 罪犯老病残信息DTO
		List<ZfLbcDto> zfLbcDtoList = new ArrayList<ZfLbcDto>();
		try {
			if(time != null && corp != null) {
				// Description: 请求Url拼接参数
				StringBuffer requestUrl = new StringBuffer("/zfxx/entity/entity_zf_lbc");
							...
							if(defaultZfJbxxArray != null && defaultZfJbxxArray.size() > 0) {
								...
								for(JsonElement defaultZfJbxxElement : defaultZfJbxxArray) {
									...
									if(zfJbxxDto != null && Util.notNull(zfJbxxDto.getCId())) {
										...
										// 罪犯老病残信息Json数组
										JsonArray defaultZfLbcArray = Util.notNull(defaultZfJbxxObject.get("RsLbcDefault_ZfLbc")) ? defaultZfJbxxObject.getAsJsonArray("RsLbcDefault_ZfLbc") : null;
										
										// 罪犯老病残信息Json数组转化为罪犯老病残DtoList
										List<ZfLbcDto> zfLbcDtoTempList = ZfLbcHttpClient.zfLbcJsonArrayToZfLbcDtoList(zfJbxxDto, defaultZfLbcArray);
										
										// 罪犯老病残信息Dto list存值
										zfLbcDtoList.addAll(zfLbcDtoTempList);
									}
								}
							}
						}
	 ...
	 * @return
	 */
	private static List<ZfLbcDto> zfLbcJsonArrayToZfLbcDtoList(ZfJbxxDto zfJbxxDto, JsonArray defaultZfLbcArray) {
		List<ZfLbcDto> zfLbcDtoList = new ArrayList<ZfLbcDto>();
		if(zfJbxxDto != null && defaultZfLbcArray != null && defaultZfLbcArray.size() > 0) {
			for(JsonElement defaultZfLbcElement : defaultZfLbcArray) {
				ZfLbcDto zfLbcDto = Util.fromJson(defaultZfLbcElement, ZfLbcDto.class);
				if(zfLbcDto != null) {
					zfLbcDto.setCZfbh(zfJbxxDto.getCZfbh());
					zfLbcDto.setCId(zfJbxxDto.getCId());
					zfLbcDto.setDUrlTime(zfJbxxDto.getDUrlTime());
					zfLbcDtoList.add(zfLbcDto);
				}
			}
		}
		return zfLbcDtoList;
