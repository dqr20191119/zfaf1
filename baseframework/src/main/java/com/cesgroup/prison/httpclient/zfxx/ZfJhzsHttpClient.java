...
import com.cesgroup.prison.httpclient.zfxx.dto.ZfJhzsDto;
...
public class ZfJhzsHttpClient {
	...
	private static ZfJhzsHttpClient currentWSHttpClient = new ZfJhzsHttpClient();
	
	private ZfJhzsHttpClient() {
		
	}
	
	private ZfJhzsHttpClient(String host) {
		ZfJhzsHttpClient.host = host;
	}
	
	public static synchronized ZfJhzsHttpClient getWSHttpClient(String host) {
		ZfJhzsHttpClient.host = host;
		return currentWSHttpClient;
	}
	
	public static synchronized ZfJhzsHttpClient getWSHttpClient() {
		if(currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfJhzsHttpClient();
	}
	...
	public static List<ZfJhzsDto> entityZfJhzs(String time, String corp, String scorp, String dept) throws RESTHttpClientException {
		// 罪犯老病残信息DTO
		List<ZfJhzsDto> zfJhzsDtoList = new ArrayList<ZfJhzsDto>();
		try {
			if(time != null && corp != null) {
				// Description: 请求Url拼接参数
				StringBuffer requestUrl = new StringBuffer("/zfxx/entity/entity_zf_jhzs");
							...
							if(defaultZfJbxxArray != null && defaultZfJbxxArray.size() > 0) {
								...
								for(JsonElement defaultZfJbxxElement : defaultZfJbxxArray) {
									...
									if(zfJbxxDto != null && Util.notNull(zfJbxxDto.getCId())) {
										...
										// 罪犯解回再审信息Json数组
										JsonArray defaultZfJhzsArray = Util.notNull(defaultZfJbxxObject.get("Default_ZfJhzs")) ? defaultZfJbxxObject.getAsJsonArray("Default_ZfJhzs") : null;
										
										// 罪犯解回再审信息Json数组转化为罪犯解回再审DtoList
										List<ZfJhzsDto> zfJhzsDtoTempList = ZfJhzsHttpClient.zfJhzsJsonArrayToZfJhzsDtoList(zfJbxxDto, defaultZfJhzsArray);
										
										// 罪犯解回再审信息Dto list存值
										zfJhzsDtoList.addAll(zfJhzsDtoTempList);
									}
								}
							}
						}
	 ...
	 * @return
	 */
	private static List<ZfJhzsDto> zfJhzsJsonArrayToZfJhzsDtoList(ZfJbxxDto zfJbxxDto, JsonArray defaultZfJhzsArray) {
		List<ZfJhzsDto> zfJhzsDtoList = new ArrayList<ZfJhzsDto>();
		if(zfJbxxDto != null && defaultZfJhzsArray != null && defaultZfJhzsArray.size() > 0) {
			for(JsonElement defaultZfJhzsElement : defaultZfJhzsArray) {
				ZfJhzsDto zfJhzsDto = Util.fromJson(defaultZfJhzsElement, ZfJhzsDto.class);
				if(zfJhzsDto != null) {
					zfJhzsDto.setCZfbh(zfJbxxDto.getCZfbh());
					zfJhzsDto.setCId(zfJbxxDto.getCId());
					zfJhzsDto.setDUrlTime(zfJbxxDto.getDUrlTime());
					zfJhzsDtoList.add(zfJhzsDto);
				}
			}
		}
		return zfJhzsDtoList;
