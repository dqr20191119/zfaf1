...
import com.cesgroup.prison.httpclient.zfxx.dto.ZfLbcDto;
import com.cesgroup.prison.httpclient.zfxx.dto.ZfRzfpDto;
...
public class ZfRzfpHttpClient {
	...
	private static ZfRzfpHttpClient currentWSHttpClient = new ZfRzfpHttpClient();
	
	private ZfRzfpHttpClient() {
		
	}
	
	private ZfRzfpHttpClient(String host) {
		ZfRzfpHttpClient.host = host;
	}
	
	public static synchronized ZfRzfpHttpClient getWSHttpClient(String host) {
		ZfRzfpHttpClient.host = host;
		return currentWSHttpClient;
	}
	
	public static synchronized ZfRzfpHttpClient getWSHttpClient() {
		if(currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfRzfpHttpClient();
	}
	...
	public static List<ZfRzfpDto> entityZfRzfp(String time, String corp, String scorp, String dept) throws RESTHttpClientException {
		// 罪犯认罪服判 信息DTO
				List<ZfRzfpDto> zfRzfpDtoList = new ArrayList<ZfRzfpDto>();
				try {
					if(time != null && corp != null) {
						// Description: 请求Url拼接参数
						StringBuffer requestUrl = new StringBuffer("/zfxx/entity/entity_zf_rzfp");
									...
									if(defaultZfJbxxArray != null && defaultZfJbxxArray.size() > 0) {
										...
										for(JsonElement defaultZfJbxxElement : defaultZfJbxxArray) {
											...
											if(zfJbxxDto != null && Util.notNull(zfJbxxDto.getCId())) {
												...
												// 罪犯认罪服判类别信息Json数组
												JsonArray defaultZfRzfpArray = Util.notNull(defaultZfJbxxObject.get("Default_ZfRzfp")) ? defaultZfJbxxObject.getAsJsonArray("Default_ZfRzfp") : null;
												
												// 罪犯认罪服判类别Json数组转化为罪犯认罪服判类别DtoList
												List<ZfRzfpDto> zfRzfpDtoTempList = ZfRzfpHttpClient.zfRzfpJsonArrayToZfRzfpDtoList(zfJbxxDto, defaultZfRzfpArray);
												
												
												// 罪犯老病残信息Dto list存值
												zfRzfpDtoList.addAll(zfRzfpDtoTempList);
											}
										}
									}
								}
					...
					throw new RESTHttpClientException("调用ZfLbcHttpClient获取罪犯老病残信息发生异常, Exception info is " + e);
				}
		return zfRzfpDtoList;
	}

	private static List<ZfRzfpDto> zfRzfpJsonArrayToZfRzfpDtoList(ZfJbxxDto zfJbxxDto, JsonArray defaultZfRzfpArray) {
		List<ZfRzfpDto> zfRzfpDtoList = new ArrayList<ZfRzfpDto>();
		if(zfJbxxDto != null && defaultZfRzfpArray != null && defaultZfRzfpArray.size() > 0) {
			for(JsonElement defaultZfRzfpElement : defaultZfRzfpArray) {
				ZfRzfpDto zfRzfpDto = Util.fromJson(defaultZfRzfpElement, ZfRzfpDto.class);
				
				// 将多个罪犯类别，合并为一个字符串，之间以英文逗号分开
				String zfRzfplb = "";
				JsonArray zfRzfplbArray = Util.notNull(defaultZfRzfpElement.getAsJsonObject().get("Default_ZfRzfplb")) ? defaultZfRzfpElement.getAsJsonObject().getAsJsonArray("Default_ZfRzfplb") : null;
				if(zfRzfplbArray != null && zfRzfplbArray.size() > 0) {
					for(JsonElement zfRzfplbElement : zfRzfplbArray) {
						String tempZfRzfplb = Util.notNull(zfRzfplbElement.getAsJsonObject().get("CRzfplb")) ? zfRzfplbElement.getAsJsonObject().get("CRzfplb").getAsString() : "";
						if(Util.notNull(tempZfRzfplb)) {
							zfRzfplb += tempZfRzfplb + ",";
						}
					}
					if(Util.notNull(zfRzfplb)) {
						zfRzfplb = zfRzfplb.substring(0, zfRzfplb.lastIndexOf(","));
					}
				}
				if(zfRzfpDto != null) {
					zfRzfpDto.setCZfbh(zfJbxxDto.getCZfbh());
					zfRzfpDto.setCId(zfJbxxDto.getCId());
					zfRzfpDto.setDUrlTime(zfJbxxDto.getDUrlTime());
					zfRzfpDto.setCRzfplb(zfRzfplb);
					zfRzfpDtoList.add(zfRzfpDto);
				}
			}
		}
		return zfRzfpDtoList;
