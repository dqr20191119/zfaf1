...
import com.cesgroup.prison.httpclient.zfxx.dto.ZfZyjwzxDto;
import com.cesgroup.prison.httpclient.zfxx.dto.ZfZyjwzxDto;
...
public class ZfZyjwzxHttpClient {
	/**
	 * 罪犯信息接口地址
	 */
	...
	private static ZfZyjwzxHttpClient currentWSHttpClient = new ZfZyjwzxHttpClient();
	
	private ZfZyjwzxHttpClient() {
		
	}
	
	private ZfZyjwzxHttpClient(String host) {
		ZfZyjwzxHttpClient.host = host;
	}
	
	public static synchronized ZfZyjwzxHttpClient getWSHttpClient(String host) {
		ZfZyjwzxHttpClient.host = host;
		return currentWSHttpClient;
	}
	
	public static synchronized ZfZyjwzxHttpClient getWSHttpClient() {
		if(currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfZyjwzxHttpClient();
	}
	...
	public static List<ZfZyjwzxDto> entityZfZyjwzx(String time, String corp, String scorp, String dept) throws RESTHttpClientException {
		// 暂予监外执行 信息DTO
				List<ZfZyjwzxDto> zfZyjwzxDtoList = new ArrayList<ZfZyjwzxDto>();
				try {
					if(time != null && corp != null) {
						// Description: 请求Url拼接参数
						StringBuffer requestUrl = new StringBuffer("/zfxx/entity/entity_zf_zyjwzx");
									...
									if(defaultZfJbxxArray != null && defaultZfJbxxArray.size() > 0) {
										...
										for(JsonElement defaultZfJbxxElement : defaultZfJbxxArray) {
											...
											if(zfJbxxDto != null && Util.notNull(zfJbxxDto.getCId())) {
												...
												// 暂予监外执行类别信息Json数组
												JsonArray defaultZfZyjwzxArray = Util.notNull(defaultZfJbxxObject.get("Default_ZfJwzx")) ? defaultZfJbxxObject.getAsJsonArray("Default_ZfJwzx") : null;
												// 暂予监外执行类别Json数组转化为暂予监外执行类别DtoList
												List<ZfZyjwzxDto> zfZyjwzxDtoTempList = ZfZyjwzxHttpClient.zfZyjwzxJsonArrayToZfZyjwzxDtoList(zfJbxxDto, defaultZfZyjwzxArray);
												// 暂予监外执行信息Dto list存值
												zfZyjwzxDtoList.addAll(zfZyjwzxDtoTempList);
											}
										}
									}
								}
					...
					throw new RESTHttpClientException("调用ZfLbcHttpClient获取暂予监外执行信息发生异常, Exception info is " + e);
				}
		return zfZyjwzxDtoList;
	}

	private static List<ZfZyjwzxDto> zfZyjwzxJsonArrayToZfZyjwzxDtoList(ZfJbxxDto zfJbxxDto, JsonArray defaultZyjwzxpArray) {
		List<ZfZyjwzxDto> zfZyjwzxDtoList = new ArrayList<ZfZyjwzxDto>();
		if(zfJbxxDto != null && defaultZyjwzxpArray != null && defaultZyjwzxpArray.size() > 0) {
			for(JsonElement defaultZfZyjwzxElement : defaultZyjwzxpArray) {
				
				
				
				ZfZyjwzxDto zfZyjwzxDto = Util.fromJson(defaultZfZyjwzxElement, ZfZyjwzxDto.class);				
				if(zfZyjwzxDto != null) {
					zfZyjwzxDto.setCZfbh(zfJbxxDto.getCZfbh());
					zfZyjwzxDto.setCId(zfJbxxDto.getCId());
					zfZyjwzxDto.setDUrlTime(zfJbxxDto.getDUrlTime());
					zfZyjwzxDtoList.add(zfZyjwzxDto);
				}
