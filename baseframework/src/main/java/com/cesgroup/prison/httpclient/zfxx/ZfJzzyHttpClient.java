...
import com.cesgroup.prison.httpclient.zfxx.dto.ZfJzzyDto;
...
public class ZfJzzyHttpClient {
	...
	private static ZfJzzyHttpClient currentWSHttpClient = new ZfJzzyHttpClient();
	
	private ZfJzzyHttpClient() {
		
	}
	
	private ZfJzzyHttpClient(String host) {
		ZfJzzyHttpClient.host = host;
	}
	
	public static synchronized ZfJzzyHttpClient getWSHttpClient(String host) {
		ZfJzzyHttpClient.host = host;
		return currentWSHttpClient;
	}
	
	public static synchronized ZfJzzyHttpClient getWSHttpClient() {
		if(currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfJzzyHttpClient();
	}
	...
	public static List<ZfJzzyDto> entityZfJzzy(String time, String corp, String scorp, String dept) throws RESTHttpClientException {
		// 罪犯老病残信息DTO
		List<ZfJzzyDto> zfJzzyDtoList = new ArrayList<ZfJzzyDto>();
		try {
							...
							if(defaultZfJbxxArray != null && defaultZfJbxxArray.size() > 0) {
								...
								for(JsonElement defaultZfJbxxElement : defaultZfJbxxArray) {
									...
									if(zfJbxxDto != null && Util.notNull(zfJbxxDto.getCId())) {
										...
										// 罪犯老病残信息Json数组
										JsonArray defaultZfJzzyArray = Util.notNull(defaultZfJbxxObject.get("Default_ZfJzzy")) ? defaultZfJbxxObject.getAsJsonArray("Default_ZfJzzy") : null;
										
										// 罪犯老病残信息Json数组转化为罪犯老病残DtoList
										List<ZfJzzyDto> zfJzzyDtoTempList = ZfJzzyHttpClient.zfJzzyJsonArrayToZfJzzyDtoList(zfJbxxDto, defaultZfJzzyArray);
										
										// 罪犯老病残信息Dto list存值
										zfJzzyDtoList.addAll(zfJzzyDtoTempList);
									}
								}
							}
						}
	 ...
	 * @return
	 */
	private static List<ZfJzzyDto> zfJzzyJsonArrayToZfJzzyDtoList(ZfJbxxDto zfJbxxDto, JsonArray defaultZfJzzyArray) {
		List<ZfJzzyDto> zfJzzyDtoList = new ArrayList<ZfJzzyDto>();
		if(zfJbxxDto != null && defaultZfJzzyArray != null && defaultZfJzzyArray.size() > 0) {
			for(JsonElement defaultZfJzzyElement : defaultZfJzzyArray) {
				ZfJzzyDto zfJzzyDto = Util.fromJson(defaultZfJzzyElement, ZfJzzyDto.class);
				if(zfJzzyDto != null) {
					zfJzzyDto.setCZfbh(zfJbxxDto.getCZfbh());
					zfJzzyDto.setCId(zfJbxxDto.getCId());
					zfJzzyDto.setDUrlTime(zfJbxxDto.getDUrlTime());
					zfJzzyDtoList.add(zfJzzyDto);
				}
			}
		}
		return zfJzzyDtoList;
