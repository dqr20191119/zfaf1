...
import com.cesgroup.prison.httpclient.zfxx.dto.ZfZdzfDto;
...
public class ZfZdzfHttpClient {
	...
	private static ZfZdzfHttpClient currentWSHttpClient = new ZfZdzfHttpClient();
	
	private ZfZdzfHttpClient() {
		
	}
	
	private ZfZdzfHttpClient(String host) {
		ZfZdzfHttpClient.host = host;
	}
	
	public static synchronized ZfZdzfHttpClient getWSHttpClient(String host) {
		ZfZdzfHttpClient.host = host;
		return currentWSHttpClient;
	}
	
	public static synchronized ZfZdzfHttpClient getWSHttpClient() {
		if(currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfZdzfHttpClient();
	}
	...
	public static List<ZfZdzfDto> entityZfZdzf(String time, String corp, String scorp, String dept) throws RESTHttpClientException {
		// 罪犯老病残信息DTO
		List<ZfZdzfDto> zfZdzfDtoList = new ArrayList<ZfZdzfDto>();
		try {
			if(time != null && corp != null) {
				// Description: 请求Url拼接参数
				StringBuffer requestUrl = new StringBuffer("/zfxx/entity/entity_zf_zdzf");
							...
							if(defaultZfJbxxArray != null && defaultZfJbxxArray.size() > 0) {
								...
								for(JsonElement defaultZfJbxxElement : defaultZfJbxxArray) {
									...
									if(zfJbxxDto != null && Util.notNull(zfJbxxDto.getCId())) {
										...
										// 罪犯老病残信息Json数组
										JsonArray defaultZfZdzfArray = Util.notNull(defaultZfJbxxObject.get("RsWwzkZdzf_ZfWwzk")) ? defaultZfJbxxObject.getAsJsonArray("RsWwzkZdzf_ZfWwzk") : null;
										
										// 罪犯老病残信息Json数组转化为罪犯老病残DtoList
										List<ZfZdzfDto> zfZdzfDtoTempList = ZfZdzfHttpClient.zfZdzfJsonArrayToZfZdzfDtoList(zfJbxxDto, defaultZfZdzfArray);
										
										// 罪犯老病残信息Dto list存值
										zfZdzfDtoList.addAll(zfZdzfDtoTempList);
									}
								}
							}
						}
	 ...
	 * @return
	 */
	private static List<ZfZdzfDto> zfZdzfJsonArrayToZfZdzfDtoList(ZfJbxxDto zfJbxxDto, JsonArray defaultZfZdzfArray) {
		List<ZfZdzfDto> zfZdzfDtoList = new ArrayList<ZfZdzfDto>();
		if(zfJbxxDto != null && defaultZfZdzfArray != null && defaultZfZdzfArray.size() > 0) {
			for(JsonElement defaultZfZdzfElement : defaultZfZdzfArray) {
				ZfZdzfDto zfZdzfDto = Util.fromJson(defaultZfZdzfElement, ZfZdzfDto.class);
				if(zfZdzfDto != null) {
					zfZdzfDto.setCZfbh(zfJbxxDto.getCZfbh());
					zfZdzfDto.setCId(zfJbxxDto.getCId());
					zfZdzfDto.setDUrlTime(zfJbxxDto.getDUrlTime());
					zfZdzfDtoList.add(zfZdzfDto);
				}
			}
		}
		return zfZdzfDtoList;
