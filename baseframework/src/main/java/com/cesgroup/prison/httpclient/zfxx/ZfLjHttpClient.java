...
/**
 * Description: 罪犯离监信息调用HttpClient工具类
 * @author lincoln.cheng
 *
 * 2019年1月17日
 */
public class ZfLjHttpClient {
	/**
	 * 罪犯信息接口地址
	 */
	...
	private static ZfLjHttpClient currentWSHttpClient = new ZfLjHttpClient();

	private ZfLjHttpClient() {
	}
	
	private ZfLjHttpClient(String host) {
		ZfLjHttpClient.host = host;
	}

	public static synchronized ZfLjHttpClient getWSHttpClient(String host) {
		ZfLjHttpClient.host = host;
		return currentWSHttpClient;
	}

	public static synchronized ZfLjHttpClient getWSHttpClient() {
		if (currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfLjHttpClient();
	}
	...
	/**
	 * Description: 获取罪犯离监信息
	 * @param time 查询日期
	 * @param corp 监狱标识
	 * @return
	 * @throws RESTHttpClientException 
	 */
	public static List<ZfJbxxDto> entityZfLj(String time, String corp) throws RESTHttpClientException {
		// 罪犯离监信息DTO
		List<ZfJbxxDto> zfJbxxDtoList = null;
		try {
			if(time != null && corp != null) {
				// Description: 请求Url拼接参数
				StringBuffer requestUrl = new StringBuffer("/zfxx/entity/entity_zf_lj");
							...
							if(defaultZfJbxxArray != null && defaultZfJbxxArray.size() > 0) {
								...
								for(JsonElement defaultZfJbxxElement : defaultZfJbxxArray) {
									...
									if(zfJbxxDto != null && Util.notNull(zfJbxxDto.getCId())) {
										...
										// 罪犯离监信息Json数组
										JsonArray defaultZfLjArray = Util.notNull(defaultZfJbxxObject.get("RsZyztbdTimeline_ZfZyztbd")) ? defaultZfJbxxObject.getAsJsonArray("RsZyztbdTimeline_ZfZyztbd") : null;
										
										// 罪犯离监信息Json数组转化为罪犯离监DtoList
										List<ZfLjDto> zfLjDtoList = ZfLjHttpClient.zfLjJsonArrayToZfLjDtoList(zfJbxxDto, defaultZfLjArray);
										
										// 将罪犯离监信息即罪犯照片信息赋予罪犯基本信息Dto
										zfJbxxDto.setZfLjDtoList(zfLjDtoList);
	...
	/**
	 * 罪犯离监信息Json数组转化为罪犯离监信息DtoList
	 * 
	 * @param defaultZfLjArray
	 * @return
	 */
	private static List<ZfLjDto> zfLjJsonArrayToZfLjDtoList(ZfJbxxDto zfJbxxDto, JsonArray defaultZfLjArray) {
		List<ZfLjDto> zfLjDtoList = new ArrayList<ZfLjDto>();
		if(zfJbxxDto != null && defaultZfLjArray != null && defaultZfLjArray.size() > 0) {
			for(JsonElement defaultZfLjElement : defaultZfLjArray) {
				ZfLjDto zfLjDto = Util.fromJson(defaultZfLjElement, ZfLjDto.class);
				if(zfLjDto != null) {
					zfLjDto.setCZfbh(zfJbxxDto.getCZfbh());
					zfLjDto.setCId(zfJbxxDto.getCId());
					zfLjDto.setDUrlTime(zfJbxxDto.getDUrlTime());
					zfLjDtoList.add(zfLjDto);
				}
			}
		}
		return zfLjDtoList;
