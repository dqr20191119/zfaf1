...
import com.cesgroup.prison.httpclient.zfxx.dto.ZfTgDto;
...
public class ZfTgHttpClient {
	...
	private static ZfTgHttpClient currentWSHttpClient = new ZfTgHttpClient();
	
	private ZfTgHttpClient() {
		
	}
	
	private ZfTgHttpClient(String host) {
		ZfTgHttpClient.host = host;
	}
	
	public static synchronized ZfTgHttpClient getWSHttpClient(String host) {
		ZfTgHttpClient.host = host;
		return currentWSHttpClient;
	}
	
	public static synchronized ZfTgHttpClient getWSHttpClient() {
		if(currentWSHttpClient != null) {
			return currentWSHttpClient;
		}
		return new ZfTgHttpClient();
	}
	...
	public static List<ZfTgDto> entityZfTg(String time, String corp, String scorp, String dept) throws RESTHttpClientException {
		// 罪犯认罪服判 信息DTO
				List<ZfTgDto> zfTgDtoList = new ArrayList<ZfTgDto>();
				try {
					if(time != null && corp != null) {
						// Description: 请求Url拼接参数
						StringBuffer requestUrl = new StringBuffer("/zfxx/entity/entity_zf_tg");
...
//											ZfJbxxDto zfJbxxDto = Util.fromJson(defaultZfJbxxObject, ZfJbxxDto.class);
											ZfTgDto zfTgDto = Util.fromJson(defaultZfJbxxObject, ZfTgDto.class);
											
											if(zfTgDto != null && Util.notNull(zfTgDto.getCId())) {
												
												zfTgDto.setDUrlTime(time);
												zfTgDtoList.add(zfTgDto);
					...
					throw new RESTHttpClientException("调用ZfLbcHttpClient获取罪犯特管等级信息发生异常, Exception info is " + e);
				}
		return zfTgDtoList;
	}
