...
$.component( "coral.form",{
	...
	_create: function () {
		...
		//创建组件外围元素
		this._initElement();
		if(this.options.formType == "custom") {
	...
	createFormLayout: function () {
		var items = this.options.items,arr=[];
		var opt = this.options;
		var rowHtml = [];
		for(var i = 0;i<items.length;i++){
			str = this.customForm(items[i]);
			arr.push(str);
		}
		var $element = $(opt.renderTo || "body").html(arr.join(""));
		$.parser.parse($element);
	},
	customForm: function(item){
		var typeStr = [];
    	if(item.columns ){
    		item = item.columns;
    		typeStr.push('<div class="row">');
    		for(var j = 0; j< item.length;j++){
    			var subItem = item[j];
    			typeStr.push("<div class=" + 'col-md-' + subItem.columnWidth+ ">");
    			if(subItem.items.length != 0){
    				var itemSub = item[j].items;
    				for(var k = 0; k< itemSub.length;k++){
    					var str = this.customForm(itemSub[k]);
    					typeStr.push(str);
    				}
    			}
    			typeStr.push("</div>");
			...
			var str = this._findEleType(item);
			typeStr.push(str);
		}
		return typeStr.join("");

	},
	_findEleType: function(itemsOption) {
		var typeStr = "";
		var type = itemsOption.xtype;
		delete itemsOption.xtype;
		//delete itemsOption.columnWidth;
		//itemsOption = $.coral.formatOption(itemsOption);
		switch (type) {
		case "autocomplete":
	    ...
	    case "textbox":
	    	itemsOption = $.coral.formatOption(itemsOption);
	    	typeStr = "<input class='coralui-"+type+"' data-options='"+ itemsOption +"'/>";
	    	break;
		case "textarea":
			itemsOption = $.coral.formatOption(itemsOption);
			typeStr = "<textarea  class='coralui-textbox' data-options='"+ itemsOption +"'/>";
			break;
		case "checkbox":
			itemsOption = $.coral.formatOption(itemsOption);
			typeStr = "<input type='checkbox' class='coralui-"+type+"' data-options='"+ itemsOption +"'/>";
	    	break;
		case "checkboxlist":
		case "radiolist":
			itemsOption = $.coral.formatOption(itemsOption);
			typeStr = "<div class='coralui-"+type+"' data-options='"+ itemsOption +"'/>";
	    	break;
		case "fileuploader":
			var buttonId = itemsOption.buttonId;
			itemsOption = $.coral.formatOption(itemsOption);
			typeStr = "<span  class='coralui-"+type+"' data-options='"+ itemsOption +"'>" +
	...
	_generateHtml: function(xtype, itemsOption, columnWidth) {
		var elementType = this._findEleType(xtype, itemsOption);
		var html = [];
		if (columnWidth) {
			html.push("<div class=" + 'col-md-' + columnWidth+ ">"+elementType +"</div>");
		} else {
			html.push("<div class='col-md-12'>"+elementType +"</div>");
		}
		return html.join("");
	},
	/**
	...
	setZoneStatus:function(zoneObj,type,isSet){
		$.coral.setZoneStatus(zoneObj,type,isSet);
	},
