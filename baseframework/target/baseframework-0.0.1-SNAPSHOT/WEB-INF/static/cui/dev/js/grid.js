...
$.grid = $.grid||{};
 var grid = $.component( "coral.grid",{
	version: "4.0.1",
	castProperties : ["rowEditButtons", "data","subGridOptions","initData","subGridRowExpanded","subGridBeforeExpand","subGridRowColapsed","shortCut", "rowList","pager","pagerTemplate", "celloptions","colModel", "picTemplate","rowattr","cellattr", "pagerContent"],
	options: {
				...
				if(this.resizing) {
					...
					if(that.options.forceFit===true){
						...
						if(that.options.footerrow) {
							$('table:first',this.sDiv).css("width",that.options.tblwidth+"px");
	...
	//	$("<div class='coral-grid-loading' id='"+"load_"+$.grid.coralID(that.options.id)+"'>" + that.options.loadtext + "</div>").appendTo( this.element );
		...
		if(this.options.treeGrid === true) {
			...
			if(that.options.datatype != "local") { 
				//that.options.localReader = {id: "_id_"};	
				that.options.localReader.id = "_id_";
			}
		}
		...
		if(this.options.rownumbers && this.options.rownumberPos == "after"){
			//设置行号
		...
		if(this.options.rownumbers && this.options.rownumberPos == "before"){
			//设置行号
			if(this.options.rownumbers) {
				this.options.colNames.unshift(that.options.rownumName);
				this.options.colModel.unshift({name:'rn',width:that.options.rownumWidth,sortable:false,resizable:false,hidedlg:true,search:false,align:'center',fixed:true});
...
//		}
	...
	_transGridView: function(){
		...
		if(that.options.footerrow) { 
			tfoot += "</tr>"; 
			if (that.options.totalRecords) {
				tfoot += "<tr role='row' class='ui-widget-content footrow totalRecords'>";
				$("th",thr).each(function ( j ) {
					tfoot += "<td role='gridcell' "+that._formatCol(j,0,'', null, '', false)+">&#160;</td>";
				})
				tfoot += "</tr>";
			}
			tfoot += "</tbody></table>"; 
	...
	_analyzeColumns:function(columns) {
		...
		for (i=0;i<this.options.colModel.length;i++){
			...
			if ( model.edittype == "combobox" || 
					...
					model.edittype == "autocomplete" || model.edittype == "combogrid" || model.edittype == "datepicker") {
	...
	_setUpEvent: function(){
		...
		$(".coral-grid-btable", that.element).add(that.pic).click(function(e) {
			...
			if(cSel === true) {
				...
				if(!isPicgrid){
					...
					//判断该行是否存在，以免报错
					if ($(that.element).find(".coral-grid-btable")[0].rows.namedItem (rowId)) {
		...
		$(".coral-grid-btable", that.element)
		// 列表上双击事件
		.dblclick(function(e) {
			td = e.target;
			ptr = $(td,that.rows).closest("tr.jqgrow");
			...
			if(scb){$(that.element).grid("setSelection",rowId,true,e);}
				...
				if(that.options.clickRowToSelect){
					$(that.options.selarrrow).each(function(i,n){
						...
						var isPicgrid = (that.options.model === "card");
						if(isPicgrid){
							that.element.find("#"+n+">ul").removeClass("coral-state-highlight").attr({"aria-selected":"false", "tabindex" : "-1"});
						}
	...
	reload: function(url){
		...
		var that = this,
	        ...
	        isUrl = false,
	        data = [];
		//极短时间内连续调用reload，需要加上url参数
		//否则1448行会删除orgdatatype，导致url不对
	...
	showHideCol : function(colname,show) {
		...
		$(this.options.colModel).each(function(i) {
			if ($.inArray(this.name,colname) !== -1 && this.hidden === sw) {
				...
				if(that.options.footerrow) { 
					var footrow = $("tr.footrow");
					footrow.each(function(j) {
						$("td:eq("+i+")", $(footrow[j]), that.grid.sDiv).css("display", show); 
					})
				}
	...
	_formatCol: function (pos, rowInd, tv, rawObject, rowId, rdata){
		...
		if ( rawObject ){
			...
			if ( cm.editable == true && cm.edittype == "datepicker" && cm.editoptions && cm.editoptions.srcDateFormat) { 
				var format = cm.editoptions.srcDateFormat,
					date = $.coral.parseDate(format, rawObjectV , that.options),
					v = $.coral.formatDate(format, date, that.options);
				dataOrg += "data-org='"+v+"'"; 
			} else if(cm.formatter == "datepicker" && cm.formatoptions &&cm.formatoptions.srcDateFormat){
				var format = cm.formatoptions.srcDateFormat,
					date = $.coral.parseDate(format, rawObjectV , that.options),
					v = $.coral.formatDate(format, date, that.options);
				dataOrg += "data-org='"+v+"'"; 
			}else {
	...
	_setPager: function (pgid, tp){
		var that = this,
			pagerTemplate1 = "<span class='paginator-left'>{viewrecords}</span><span class='paginator-center'>{prev}{links}{next}{pginput}{navbar}{description}</span><span class='paginator-right'><em>"+that.options.pagerContent.rowlist+"</em>{rninput}<em>"+that.options.pagerContent.rowlistSuffix+"</em></span>",
	...
	var pagerTemplate = false;
		...
		var pginp = "", 
			...
			pginput = "<em class='page-item'>"+that.options.pagerContent.links+"</em><input type='text' class='coral-pg-input'/><em class='page-item'>"+that.options.pagerContent.linksSuffix+"</em>",
			reload = "<button class='reload-button' id='_reloadGrid' type='button' " +
					"data-options='customData:{gridId:\""+that.options.id+"\"}," +
					"label:\""+that.options.pagerContent.reload+"\",text: false,icons: \"cui-icon-loop3\",onClick: function( e, ui ){" +
					"$( \"#"+that.options.id+"\" ).grid( \"reload\" );" +
				"}'>reload</button>",
			first = "<span id='"+po[0]+"' class='page-item coral-paginator-first coral-state-default coral-corner-all' tabindex='0'><span class='cui-icon-first2' title='"+that.options.pagerContent.first+"'></span></span>",
			last = "<span id='"+po[3]+"' class='page-item coral-paginator-last coral-state-default coral-corner-all' tabindex='0'><span class='cui-icon-last2' title='"+that.options.pagerContent.last+"'></span></span>",
			next = "<span id='"+po[2]+"' class='page-item coral-paginator-next coral-corner-all' tabindex='0'><span class='cui-icon-arrow-right3' title='"+that.options.pagerContent.next+"'></span></span>",
			prev = "<span id='"+po[1]+"' class='page-item coral-paginator-prev coral-corner-all' tabindex='0'><span class='cui-icon-arrow-left3' title='"+that.options.pagerContent.prev+"'></span></span>",
			sep = "<span>|</span>",
			navbar = "<span class='page-item pager-navbar'></span>",
			toolbar = "<div class='pager-toolbar'></div>",
		...
		that.options.toolbarOptions = pagerOpts.toolbarOptions || that.options.toolbarOptions;
		that.options.description = pagerOpts.description;
		var navbarOptions = that.options.navbarOptions || {};
		pagerOpts.toolbarOptions = pagerOpts.toolbarOptions || that.options.toolbarOptions || {};
...
*/			if (that.options.viewrecords){
		...
		if (that.options.autoWrap) {
			that.element.find(".coral-row-ltr").find("td").css({"white-space":"normal","word-break":"break-all"});
		}
	...
	_populate: function (npage) {
		...
		if(true) {//上次请求未完成则不能继续
					...
					try {
						...
						if( that.options.connectGridId == null) {
							...
							for(var i=0;i<connectGridId.length;i++) {
								id[i] = $.trim(connectGridId[i]);
								connectItem.push("#"+id[i]+" tbody:first");
							}
							connectItem = connectItem.join(", ");
						}
						$("#" + that.element[0].id).grid("sortableGrid",{connectWith:connectItem, connectGridId:id, containment: that.options.containment});
					} catch (e){}
				}
			};
			var adjust = 0;
			...
			var success = function ( content, st, xhr ){
				...
				//var dReader, drows;
				//getAccessor只适用于原始的json格式，对于treeGrid不适用，故在_addJSONData里再处理
				/*
	...
	_addSingle: function(rowid,pos,irow,checked){
		var that = this,
			isPicgrid = !!(that.options.model == "card"),
			v = "<input role=\"radio\" type=\"radio\""+" id=\"jqg_"+that.options.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+that.options.id+"\"" + (checked ? "checked=\"checked\"" : "")+"/>",
			prp = that._formatCol( pos,irow,'',null, rowid, true);
		if(isPicgrid)return v;
		return "<td role=\"gridcell\" "+prp+">"+v+"</td>";
	},
...
//		}
		...
		if(coreData.length>0 && itemJson.coreData){
	...
	_addJSONData: function(data,t, rcnt, more, adjust) {
		...
		while (i<len) {
			...
			rowData.push("");
			if(that.options.rownumbers && that.options.rownumberPos == "before"){
				if( ni ) {// ni为行号
			...
			if(that.options.rownumbers && that.options.rownumberPos == "after"){
				if( ni ) {// ni为行号
					rowData.push( that._addRowNum(0,i,that.options.page,that.options.rowNum) );
				//	orgRowData.push("rowNum");
				}
			...
			//getAccessor 只适用原始的json格式；treegrid的data格式不适用
		...
		if(that.options.model == "card" && that.options.cardNumber){
			var width = $("#"+$.grid.coralID(that.options.id)+" .coral-pic").find("li.gridPanel").outerWidth();
			$("#"+$.grid.coralID(that.options.id)+" .coral-pic").css("width",(width+3)*that.options.cardNumber+"px");
			$("#"+$.grid.coralID(that.options.id)).css("overflow","auto");
		}
	...
	setGroupHeaders: function ( o ) {
		...
		$trLabels = $htable.children("thead").children("tr:last").addClass("jqg-second-row-header"),
		...
		for (i = 0; i < cml; i++) {
			...
			if (iCol >= 0) {
				...
				$colHeader = $('<th>').attr({role: "columnheader"})
					...
					.css({/*'height':'22px', */'border-bottom': '0px none'})
					.html(titleText);
	...
	_setGridWidth : function(nwidth, shrink) {
		...
		if(shrink===true || useMinWidth) {
			//if ( useMinWidth ) {
				...
				var groupHeader = $("tr.jqg-first-row-header>th:eq("+lvc+")", $('table:first',that.grid.columnsView));
				if(groupHeader.length>0){groupHeader[0].style.width = cw+"px";}
	...
	_setGridHeight : function (nh) {
		...
		if(String(nh).toLowerCase() === "auto") { 
			...
			tblHeight = nh - $(grid.columnsView).outerHeight(true) 
				...
				- $(grid.caption).height()
				- $(grid.sDiv).height();     //计算footerData行的高度
		...
		if(rdata) {
			...
			while(k < datalen) {
				...
				for(i = gi+si+ni; i < t.options.colModel.length;i++){
					...
					lcdata[nm] = data[nm];
					v = (t.options.onSortableLoad && rowid.indexOf("jqempty_") == 0)? "" :t._formatter( rowid, $.grid.getAccessor(data,nm), i, data);
	...
	footerData: function(data, format) {
		...
		//this.each(function(){
			...
			success=true;
			if (t.options.totalRecords) {
				if (data.totalRecords) {
					var totalData = data.totalRecords;
					$(this.options.colModel).each(function(i){
						nm = this.name;
						if(action === "set") {
							if( totalData[nm] !== undefined) {
								vl = format ? t._formatter( "", totalData[nm], i, totalData, 'edit') : totalData[nm];
								title = this.title ? {"title":$.grid.stripHtml(vl)} : {};
								$("tr.totalRecords td:eq("+i+")",t.grid.sDiv).html(vl).attr(title);
								success = true;
							}
						} else if(action === "get") {
	...
	sortableGrid: function(opts) {
		var	that = this;
		that.options.dragHighlight = that.options.dragHighlight == true?"coral-state-highlight":"";
		var firstGrid,  //被拖拽的grid
			selectData=[], selectDataId=[], childData = [], expanded = true,
			originalPermutationlInnerListId; //被拖拽的grid的初始list顺序
		$("tbody:first",that.element).sortable({
			appendTo: 'body',
			containment: opts.containment,    //设置拖动的边界；为window时只能在当前浏览器窗口的内容区域拖动
			connectWith: opts.connectWith,
			placeholder:that.options.dragHighlight,
			dropOnEmpty: true,
			helper: function(e ,ui) {
				var html = $("<div></div>");
				firstGrid = $(e.target).closest(".ctrl-init-grid ").attr("id");
				if (that.options.treeGrid) {
					var parentId =  $(e.target).parents("tr").attr("id");
						parentData = $("#"+firstGrid).grid("getRowData", parentId);
					selectData = $("#"+firstGrid).grid("getFullTreeNode", parentData);
					childData = $("#"+firstGrid).grid("getNodeChildren", parentData);
					for (var i=0;i<selectData.length;i++) {
						selectDataId.push(selectData[i]["_id_"]);
						html.append($("#"+selectData[i]["_id_"], $("#"+firstGrid)).clone());
					}
					if ($("#"+parentId).children("td").find(".tree-plus").length != 0) {
						expanded = false;
					}
				} else if (that.options.multiselect) {
			...
			start: function(e, ui) {
				if (that.options.treeGrid) {
					...
					pos = $("#"+firstGrid).grid("getInd", itemId);
					for (var i=0;i<selectData.length;i++) {
						$("#"+selectData[i]["_id_"], $("#"+firstGrid)).css('display','none');
					}
				} else if(that.options.multiselect) {
					for (var i=0;i<selectDataId.length;i++) {
						$("#"+selectDataId[i], $("#"+firstGrid)).css('display','none');
					}
				}
				//新增一条空数据撑出placeholder
				if (opts.connectGridId) {
					if (opts.connectGridId.length == 1) {
						if (opts.connectGridId[0] != firstGrid) {
							that.refreshSortableGrid(opts.connectGridId);
						}
					} else {
						that.refreshSortableGrid(opts.connectGridId);
					}
				}
				if(that.options.dragHighlight){
					ui.item.prev("tr").children("td").css("border-bottom","0px");
					ui.placeholder.children("td").css("border-bottom","0px");
				}
				that._trigger("onSortableStart", e, {originalPermutation:originalPermutationlInnerListId});
			},
			change:function(e,ui){
				if(that.options.dragHighlight){
					ui.placeholder.prev().children("td").css("border-bottom","0px");
					ui.placeholder.children("td").css("border-bottom","0px");
					ui.placeholder.children("td:last").css("border-right","1px solid #fed22f");
				}
			},
			//stop时prevItemId和nextItemId取值以ui.item为准，会出现prevItemId/nextItemId在selectDataId中，需要先删除
			beforeStop: function(e, ui) {
				if(that.options.multiselect) {
					if (selectData.length) {
						for (var i=0;i<selectDataId.length;i++) {
							if (ui.item.attr("id") != selectDataId[i]) {
			...
			stop: function(e ,ui) {
				var currentGrid = ui.item.closest(".ctrl-init-grid ").attr("id"),
					...
					position = "last",
					selection = $("#"+firstGrid).grid("getDataIDs");
				if (prevItemId) {
					if (prevItemId.indexOf("jqempty_")!=-1) {
						prevItemId = undefined;
					}
				}
				if (nextItemId) {
					if (nextItemId.indexOf("jqempty_")!=-1) {
						nextItemId = undefined;
					}
				}
				if (that.options.treeGrid) {
					var itemId = ui.item.attr("id"), 
						data = $("#"+currentGrid).grid("getDataIDs"),
					    visibleData = that._getShowData(data),
					    index = $.inArray(itemId, visibleData);
					prevItemId = visibleData[index-1]? visibleData[index-1]:  undefined;
					nextItemId = visibleData[index+1]? visibleData[index+1]: undefined;
				} else {
					if ( that.options.multiselect) {
						...
						if(targetGrid != currentGrid) {
							...
							$("#" + firstGrid+ " tbody:first").append($("#"+ui.item.attr("id")).clone()); 
							$("#"+ firstGrid).grid("delRowData", ui.item.attr("id"));
							$("#"+ui.item.attr("id"), $("#"+firstGrid)).remove();
						}
						if ( !selectData.length) {
							if (prevItemId) {
								position = "after";
							...
							for (var i=0;i<selectData.length;i++) {
								...
								var selectResult = $("#"+currentGrid).grid("option", "multiselect");
								if (selectResult) {
				...
				var tmpGird=[];
				tmpGird.push(firstGrid);
				var	grid = opts.connectGridId? opts.connectGridId : tmpGird;
				for (var k=0;k<grid.length;k++) {
					$("#"+ grid[k]).grid("delRowData", "jqempty_"+k);
					$("tbody:first", "#"+grid[k]).sortable( "refreshPositions" );
					$("#"+grid[k]).grid("refresh");
				}
				if (that.options.treeGrid) {
					that._trigger("onSortableStop", e, {datafrom:targetGrid, currentGrid:currentGrid, itemId:selectDataId, prevItemId:prevItemId, nextItemId:nextItemId});
				} else {
					that._trigger("onSortableStop", e, {datafrom:targetGrid, currentGrid:currentGrid, itemId:selectDataId, prevItemId:prevItemId, nextItemId:nextItemId, originalPermutation:originalPermutationlInnerListId});
				}
				if (!selectData.length) {
					var tmp = selectDataId;
					selectDataId = [];
					selectDataId.push(tmp);
				}
				var sel = $("#" + currentGrid).grid("getDataIDs");
				//if ( that.options.multiselect ) {
					var oriSel =  $("#" + firstGrid).grid("getDataIDs"),
						oriSelarrrow = $("#" + firstGrid).grid("option","selarrrow").concat(),
						selarrrow =  $("#" + currentGrid).grid("option","selarrrow").concat();
					if (oriSel.length == oriSelarrrow.length && oriSel.length) {
						$('#cb_'+firstGrid, ".coral-grid-columns-view").prop("checked", true);
					} else {
						$('#cb_'+firstGrid, ".coral-grid-columns-view").prop("checked", false);
					}
					if (sel.length == selarrrow.length) {
						$('#cb_'+currentGrid, ".coral-grid-columns-view").prop("checked", true);
					} else {
						$('#cb_'+currentGrid, ".coral-grid-columns-view").prop("checked", false);
					}
				//}
				...
				expanded = true;
				if(that.options.dragHighlight){
					var $currentGrid = $("#"+currentGrid).find(".coral-grid-btable").find(".jqgrow"),
						$targetGrid = $("#"+targetGrid).find(".coral-grid-btable").find(".jqgrow");
					var targetArr = $currentGrid.children("td"),
					currentArr = $targetGrid.children("td");
					for(var i = 0; i <= targetArr.length; i++){
						$(targetArr[i]).css("border-bottom","1px solid #ededed");
					}
					for(var j = 0; j <= currentArr.length; j++){
						$(currentArr[j]).css("border-bottom","1px solid #ededed");
					}
				}
			},
			//删除原本grid里面被选中的数据
	...
	//增加一行空数据撑出placeholder
	refreshSortableGrid: function(grid) {
		for (var k=0;k<grid.length;k++) {
			var height = $("#"+grid[k]).height();
			$("#"+grid[k]).grid("addRowData", "jqempty_"+k, {}, "last");
			var tbodyHeight = $("tbody:first", "#"+grid[k]).height();
			$("#jqempty_"+k).css("height",height-tbodyHeight);
			$("#jqempty_"+k).children("td").css("border","0");
			$("#jqg_"+ grid[k]+"_jqempty_"+k).css("display","none");
			$("#"+grid[k]).grid("refresh");
			$("tbody:first", "#"+grid[k]).sortable( "refreshPositions" );
		}
	},
	_getShowData: function(data) {
		var array=[], index; 
		for (var i=0;i<data.length;i++) {
			if($("#"+data[i]).css("display") !='none') {
				array.push(data[i])
			}
		}
		return array;
	...
	gridDnD : function(opts) {
		var that = this,selectData=[], selectDataId=[], html;
		if(!that.grid) { return; }
		// Currently we disable a treeGrid drag and drop
		if(that.options.treeGrid) { return; }
		if(!$.fn.draggable || !$.fn.droppable) { return; }
...
grid = $.component( "coral.grid", $.coral.grid, {
	...
	transformToTreeFormat: function(sNodes) {
		var i, l, level,
		key = this.options.keyName?this.options.keyName: "id",
	...
	getNodeParent : function(rc) {
		var result = null;
		var that = this;
		//2016-1-13 修改所有rc[localKey] 
		var localKey = "_id_";
		rc[localKey] = rc[that.options.keyName? that.options.keyName: "id"];
		if(!that.grid || !that.options.treeGrid) {return;}
		switch (that.options.treeGridModel) {
			case 'nested' :
	...
	getFullTreeNode : function(rc) {
		var result = [];
		//this.each(function(){
			var $t = this, that=$t, len;
			var localKey = "_id_";
			rc[localKey] = rc[$t.options.keyName?$t.options.keyName: "id"];
	...
	expandNode : function(rc) {
		...
		var localKey = "_id_";
		rc[localKey] = rc[this.options.keyName? this.options.keyName: "id"];
	...
	delTreeNode : function (rowid) {
		var $t = this, rid = $t.options.localReader.id,
		left = $t.options.treeReader.left_field,
		right = $t.options.treeReader.right_field, myright, width, res, key;
		if(!$t.grid || !$t.options.treeGrid) {return;}
		var rc = $t.options._index[rowid];
		if (rc !== undefined) {
			// nested
			myright = parseInt($t.options.data[rc][right],10);
			width = myright -  parseInt($t.options.data[rc][left],10) + 1;
			var dr = $t.element.grid("getFullTreeNode",$t.options.data[rc]);
			if(dr.length>0){
				for (var i=0;i<dr.length;i++){
					$t.element.grid("delRowData",dr[i][rid]);
				}
			}
			if( $t.options.treeGridModel === "nested") {
				// ToDo - update grid data
				res = $.grid.from($t.options.data)
	...
	addChildNode : function( nodeid, parentid, data ) {
		...
		if(data) {
			...
			var prow = $t.element.grid('getInd', parentid);
				leaf = false;
				...
				if ( parentid === undefined  || parentid === null || parentid==="") {
					...
					var childs = $t.element.grid('getFullTreeNode', parentdata);
					// if there are child nodes get the last index of it
					if(childs.length) {
						i = childs[childs.length-1][that.options.localReader.id];
						rowind = i;
						i = $t.element.grid('getInd',rowind)+1;
					} else {
						i = $t.element.grid('getInd', parentid)+1;
					}
					// if the node is leaf
			...
			if( that.options.treeGridModel === "nested") {
				...
				if(parentid !== null) {
					...
					maxright = parseInt( $t.element.grid('getCol', right, false, 'max'), 10);
			...
			if( parentid === null || $t.element.grid("isNodeLoaded",parentdata) || leaf ) {
				$t.element.grid('addRowData', nodeid, data, method, rowind);
				$t.element.grid('setTreeNode', i, len);
			}
	...
	showGridHeader: function(){
		...
		//处理无数据时的列冻结:
		//datatype为json，存在延迟，先进行showGridHeader，再调_endReq，无数据时正常隐藏
		//datatype为local，先_endReq内进行了隐藏，在此处又被show出来，故再次进行隐藏	
		if ($(this.grid.rowsView).find(".jqgrow").length == 0) {
			$(this.grid.fhDiv).css("display","none");
		}
	},
	//grid校验模块
	...
	setFrozenColumns : function () {
		...
		if( frozen) {
			...
			$($t.element).bind('gridaftercomplete.setFrozenColumns', function () {
				...
				var btbl, rightbtbl;
				if( maxfrozen>=0 && $t.options.data.length !=0) {
...
grid = $.component( "coral.grid", $.coral.grid, {
	...
	groupingRender : function (grdata, colspans, page, rn ) {
		...
		$.each(grp.groups,function(i,n){
			...
			var grpTextStr = '',tempData = that.options.data; 
			if($.isFunction(grp.groupText[n.idx])) { 
				grpTextStr = grp.groupText[n.idx].call(this,tempData, gv, n.cnt, n.summary);
			} else {
				grpTextStr = $.grid.template(grp.groupText[n.idx], gv, n.cnt, n.summary);
			}
			...
			if(grp.groupSummaryPos[n.idx] === 'header')  {
				...
				str += "<tr id=\""+hid+"\"" +(grp.groupCollapse && n.idx>0 ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"coral-component-content jqgroup coral-row-"+that.options.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\" colspan=\""+(grp.groupColumnShow[n.idx] === false ? colspans-1 : colspans)+"\">"+ icon + grpTextStr +"</td></tr>";
			}
...
grid = $.component("coral.grid",$.coral.grid,{
	...
	expandSubGridRow : function(rowid) {
		...
		if($t.options.subGrid===true) {
			var rc = $t.element.grid("getInd",rowid,true);
			if(rc) {
	...
	filterToolbar : function(p){
		...
		var triggerToolbar = function() {
			...
			if(!bsr) { 
				...
				$(that.element[0]).grid("option",{search:sd}).grid("reload", {page:1}); 
