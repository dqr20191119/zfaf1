...
$.component( "coral.autocomplete", $.coral.inputbase,{
	...
	options: {
		...
		valueField: "value",
		textField: "label",
		searchField:"value",  //支持多个值搜索，默认搜索valueField
		// callbacks
		onValidSuccess: null,
		onValidError: null,
		onChange: null,
	...
	_initAutocomplete: function(){
		...
		if ( this.options.width) {				
			this.textboxWrapper.css( "width", this.options.width );
		}
		...
		this._on({
			...
			keydown: function( event ) {
				...
				case keyCode.ENTER:
					...
					}else if(this.options.autoSelect == true){
						var selectEle = this.menu.active = this.menu.element.children(":visible").eq(0);
						selectEle.addClass("coral-state-focus");
						this.menu.select( event );
					}
					break;
				case keyCode.TAB:
		...
		this._on( this.menu.element, {
			...
			menuonselect: function( event, ui ) {
				...
				if ( false !== this._trigger( "onSelect", event, { item: item } ) ) {
						//var item = ui.item.data( "coral-autocomplete-item" );
						var item = this.options.tempData[ui.item.data( "key" )];
						valueField = item!=undefined ? item[this.options.valueField]: [];
						textField =  item!=undefined ? item[this.options.textField]:[];
...
//				}
	...
	_setWidth: function( value) {
		if (value) {
			this.textboxWrapper.css( "width", value );
			this._resizeMenu();
		}
	},
