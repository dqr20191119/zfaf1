...
@Repository
@Scope("prototype")
public class JdbcProductionDAO {
	// the default JdbcTemplate. if u want change the property of JdbcTemplate,
	// create a new JdbcTemplate from DataSource.
	private static final Logger logger = LoggerFactory.getLogger(JdbcProductionDAO.class);
	@Resource
...
/*	@PostConstruct
	public void init(){
		productionDataSource=(DataSource)ApplicationContextUtil.getInstance().getBean("productionDataSource");
		//productionJdbcTemplate = new JdbcTemplate(productionDataSource);
	}*/
	...
	public int[] batchUpdate(String sql, List<Object[]> batchArgs) {
		return productionJdbcTemplate.batchUpdate(sql, batchArgs);
	}
	...
	private SimpleJdbcCall createJdbcCallForFunction(String pacakgeName, String functionName) {
		...
		JdbcTemplate myJdbcTemplate = new JdbcTemplate(productionDataSource);
	...
	public int[] executeBatchInsert(String schemaName, String tableName, List<Map<String, Object>> args) {
		SimpleJdbcInsert insert = new SimpleJdbcInsert(productionDataSource).withSchemaName(schemaName).withTableName(tableName);
	...
	public JdbcTemplate newJdbcTemplate() {
		return new JdbcTemplate(productionDataSource);
	}
	
	public JdbcTemplate getJdbcTemplate() {
		return this.productionJdbcTemplate;
	}
	...
	public List<Map<String, Object>> query(int maxRows, String sql, Object... args) {
		//DataSource dataSource1=(DataSource)ApplicationContextUtil.getInstance().getBean("productionDataSource");
		// the jdbcTemplate is a default object, so you could not change it
		JdbcTemplate myJdbcTemplate = new JdbcTemplate(productionDataSource);
		//JdbcTemplate myJdbcTemplate = new JdbcTemplate(dataSource1);
...
/*	@Autowired
	public void setDataSource(DataSource dataSource) {
		this.productionDataSource = dataSource;
		this.productionJdbcTemplate = new JdbcTemplate(dataSource);
	}

	public DataSource getDataSource(){
		return this.productionDataSource;
	}*/
	...
	public int update(String sql, Object... args) {
		return productionJdbcTemplate.update(sql, args);
	}
	
	public boolean batchUpdate(String sql,final Integer[] objs){
		try {
			productionJdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
