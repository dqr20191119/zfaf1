...
$.component( "coral.tabs", {
	...
	options: {
		...
		show: null,
		loadtext:"加载中，请耐心等候...",
		overflowContent: false,   //ul宽度超出整个tabs后是否收缩，true时显示箭头，点击箭头切换。false表示不收缩，换行显示
		// 回调函数
		/*activate: null,
		beforeActivate: null,
		beforeLoad: null,
	...
	_setOption: function( key, value ) {
		if ( key === "active" ) {
			...
			if (typeof value === "string") {				
				...
				this._activate( this.getIndexById ( value) );
			} else {
	...
	_refresh: function() {
		...
		if (this.options.overflowContent) {
			this._showhideArrow();
		}
	},
	//增加Tab
	add: function(option) {
		...
		if (!isUrl) {
			...
			if (this.options.overflowContent) {
				this.component().children( ".coral-tabs-content" ).children(".coral-tabs-nav").append( li );
			} else {
				this.component().children( ".coral-tabs-nav" ).append( li );
			}
			// for tabs at bottom the content must before the nav
			if(this.element.hasClass("coral-tabs-bottom")){
				if (this.options.overflowContent) {
					this.component().children( ".coral-tabs-content" ).before( "<div id='" + panelId + "'>"+content+"</div>" );
				} else {
					this.component().children( ".coral-tabs-nav" ).before( "<div id='" + panelId + "'>"+content+"</div>" );
				}
			}else{
		...
		if(hide == true){
			li.hide();
			//this.component().find("div#"+tabId).hide();
		}
      	if ( typeof this.options.newbtn == "boolean" && this.options.newbtn ) {
      		this.addnewbtn();
		}
	...
	_remove: function(index) {
		...
		if(removeIndex != -1 && removeIndex !== false){
	...
	showTabs: function(index,isShow){
		if (index instanceof Array) { // array of index or id
			for (var i in index) {
				this._showTabs(index[i],isShow);
			}
		} else {
			this._showTabs(index,isShow);
		} 
		//this.refresh();
	},
	_showTabs: function(index,isShow){
		var removeIndex;
		...
		if(removeIndex != -1){
			var removeId=this._getList().find( "> li:has(a[href])" ).eq(removeIndex).attr("aria-controls");
			if(isShow == false){
				this.component().find("li[aria-controls='"+removeId+"']").hide();
				this.component().find("div#"+removeId).hide();
			}else{
				this.component().find("li[aria-controls='"+removeId+"']").show();
				//this.component().find("div#"+removeId).show();
			}
		}
	},
	//lihaibo add begin
	...
	_processTabs: function() {
		...
		if ( this.options.overflowContent && !this.tablist.parent().hasClass("coral-tabs-content")) {
			this.tablist.wrap("<div class='coral-tabs-content coral-tabs'></div>");
			this.element.prepend("<div class='icon-arrow-left3 coral-scroller-left coral-tabs-arrow'></div>"
								+"<div class='icon-arrow-right3 coral-scroller-right coral-tabs-arrow'></div>");
			var tabsClass = this._getClass(this.options.cls); 
			if (tabsClass == "coral-tabs-right" || tabsClass == "coral-tabs-left") {
				this.element.children(".coral-scroller-left").removeClass("icon-arrow-left3").addClass("icon-arrow-up3");
				this.element.children(".coral-scroller-right").removeClass("icon-arrow-right3").addClass("icon-arrow-down3");
			} 
		...
		this.anchors.each(function( i, anchor ) {
			...
			// 远程 tab
				...
				if ( !panel.length ) {
					...
					// modify by mengshuai begin
					var cls = that._getClass(that.options.cls),
						tabsContent = that.tablist.parent(".coral-tabs-content").length? that.tablist.parent(".coral-tabs-content") : undefined;
					if(cls == "coral-tabs-bottom"){
						panel.insertBefore( that.panels[ i - 1 ] || tabsContent || that.tablist );
					}else{
						panel.insertAfter( that.panels[ i - 1 ] || tabsContent || that.tablist );
					}
					// modify by mengshuai end
				}
	...
	_setupDisabled: function( disabled ) {
		...
		if ( this.options.overflowContent) {
			if (disabled === true) {
				this.element.children(".coral-tabs-arrow").addClass("coral-state-disabled");
			} else {
				this.element.children(".coral-tabs-arrow").removeClass("coral-state-disabled");
			}
		}
	},
	...
	_setupHeightStyle: function( heightStyle ) {
		var maxHeight,
			...
			isOverflow = this.options.overflowContent;
		...
		if ( heightStyle === "fill" ) {
			...
			this.element.children().not( this.panels ).each(function() {
				...
				//如果设置箭头切换，就不需要计算箭头的高度
				if (!isOverflow || (isOverflow && !$(this).hasClass("coral-tabs-arrow"))) {
	 ...
	 * 根据 index 获取 id
	 */
	//根据aria-controls获取，若动态添加后获取(li指定了id)，则id不对
	...
	getIndexById: function ( id ) {
		...
		var index  = lis.index( lis.filter( "[aria-controls$='" + id + "']" ) );
		if(index == -1){
			index = lis.index( lis.filter( "[id$='" + id + "']" ) )
		}
		return index;
	},
	//若li有id，则获取
	...
	load: function( index, event ) {
		...
		//event判断是否为外部手动调用
		if ( (that._isLocal( anchor[ 0 ] ) || $.data( anchor[0], "cache") ) && !(typeof event === "boolean")) {
			return ;
	...
	_getLiWidth: function() {
		var tabWidth = {},
			ulTarget = this.element.children(".coral-tabs-content").children(".coral-tabs-nav");
		tabWidth.totalWidth = 0;
		tabWidth.width = [];
		$(".coral-corner-top", ulTarget).each(function() {
			tabWidth.totalWidth += $(this).outerWidth(true);
			tabWidth.width.push(tabWidth.totalWidth);
		})
		return tabWidth;
	},
	_getLiHeight: function() {
		var tabHeight = {},
			ulTarget = this.element.children(".coral-tabs-content").children(".coral-tabs-nav");
		tabHeight.totalHeight = 0;
		tabHeight.height = [];
		$(".coral-corner-top", ulTarget).each(function() {
			tabHeight.totalHeight += $(this).outerHeight(true);
			tabHeight.height.push(tabHeight.totalHeight);
		})
		return tabHeight;
	},
	//是否显示切换箭头
	_showhideArrow: function() {
		var cls = this._getClass(this.options.cls),
			content = this.element.children(".coral-tabs-content");
		if ( cls == "coral-tabs-left" || cls == "coral-tabs-right") {
			var height = this.element.height(),
				liHeight = this._getLiHeight().totalHeight;
			if (liHeight > height) {
				content.addClass("coral-content-row").css("height", height-40);
				this.element.children(".coral-tabs-arrow").show();
				this._setArrowPosition(cls);
			} else {
				this.element.children(".coral-tabs-arrow").hide();
				content.removeClass("coral-content-row").css("height", height);
				content.children(".coral-tabs-nav").css({marginTop: 0});
			}
		} else {
			var contentWidth = content.width(),
				liWidth = this._getLiWidth().totalWidth;
			if (liWidth > contentWidth) {
				content.addClass("coral-content-row");
				this.element.children(".coral-tabs-arrow").show();
				this._setArrowPosition(cls);
			} else {
				content.removeClass("coral-content-row");
				this.element.children(".coral-tabs-arrow").hide();
				content.children(".coral-tabs-nav").css({marginLeft: 0});
			}
		}
	},
	_getClass: function(cls) {
		if (cls) {
			var tempcls = cls.split(" ").reverse(),
				overflowCls = ["coral-tabs-left", "coral-tabs-right", "coral-tabs-bottom"],
				existClass;
			for (var i=0;i<tempcls.length;i++) {
				if ($.inArray(tempcls[i], overflowCls) > -1) {
					existClass = tempcls[i];
					break;
				}
			}
			return existClass;
	...
	_bindArrowEvent: function() {
		var that = this,
			tabsClass = this._getClass(this.options.cls),
			element = this.element,
			rowCount = 0,
			colCount = 0;
		this.element.children(".coral-tabs-arrow").off("click").on("click", function(e) {
			if (that.options.disabled === true) {
 			...
 			var target = e.target,
				...
				offset,
				tabWidth = that._getLiWidth(),
				tabHeight = that._getLiHeight(),
				width = tabWidth.width,
				LiWidth = tabWidth.totalWidth,
				height = tabHeight.height,
				LiHeight = tabHeight.totalHeight,
				contentWidth = element.children(".coral-tabs-content").width(),
				contentHeight = element.children(".coral-tabs-content").height();
			if (tabsClass == "coral-tabs-left" || tabsClass == "coral-tabs-right") {
				marginType = "marginTop";
			} else {
				marginType = "marginLeft";
			}
			var ulTarget = $(element).children(".coral-content-row").children(".coral-tabs-nav");
			offset  = parseInt(ulTarget.css(marginType));
			var offsetValue = Math.abs(offset);
			if ($(target).hasClass("icon-arrow-left3")) {
				rowCount = rowCount - 1;
				if (width[rowCount-1]) {
					ulTarget.css({marginLeft:'-' + width[rowCount-1] + 'px'});
				} else {
					ulTarget.css({marginLeft: 0});
					rowCount = 0;
				}
			} else if ($(target).hasClass("icon-arrow-right3")) {
				var valueWidth = contentWidth + offsetValue;
				if (LiWidth > valueWidth) {
					ulTarget.css({marginLeft:'-' + width[rowCount] + 'px'});
					rowCount++;
				}
			} else if ($(target).hasClass("icon-arrow-up3")){
				colCount = colCount - 1;
				if (height[colCount-1]) {
					ulTarget.css({marginTop:'-' + height[colCount-1] + 'px'});
				} else {
					ulTarget.css({marginTop: 0});
					colCount = 0;
				}
			} else if ($(target).hasClass("icon-arrow-down3")) {
				var valueHeight = contentHeight + offsetValue;
				if (LiHeight > valueHeight) {
					ulTarget.css({marginTop:'-' + height[colCount] + 'px'});
					colCount++;
				}
			}
	...
	_setArrowPosition: function(cls) {
		var element = this.element,
			contentUL = element.children(".coral-tabs-content").children(".coral-tabs-nav");
		if (cls == "coral-tabs-left" || cls=="coral-tabs-right") {
			var left = contentUL.width()/2,
				upArrow = element.children(".coral-tabs-arrow.icon-arrow-up3"),
				downArrow = element.children(".coral-tabs-arrow.icon-arrow-down3"),
				upWidth = upArrow.width()/2,
				downWidth = downArrow.width()/2;
		} else {
			var top = contentUL.height()/2,
				leftArrow = element.children(".coral-tabs-arrow.icon-arrow-left3"),
				rightArrow = element.children(".coral-tabs-arrow.icon-arrow-right3"),
				leftHeight = leftArrow.height()/2,
				rightHeight = rightArrow.height()/2;
		}
		switch (cls) {
			case "coral-tabs-left":
				upArrow.css({
					left: left - upWidth
				});
				downArrow.css({
				...
				break;
			case "coral-tabs-right":
				upArrow.css({
					right: left - upWidth
				});
				downArrow.css({
					right: left - downWidth
				});
				break;
			case "coral-tabs-bottom":
				leftArrow.css({
					bottom: top - leftHeight
				});
				rightArrow.css({
					bottom: top - rightHeight
				});
				break;
			default:
				leftArrow.css({
