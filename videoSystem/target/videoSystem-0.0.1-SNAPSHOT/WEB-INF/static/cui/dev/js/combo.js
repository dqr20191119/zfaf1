...
$.component("coral.combo", $.coral.inputbase, {
	...
	options: {
		...
		filter: function(q, row) {
			...
			//搜索时下拉面板只显示text，不需要比较value（对于用户来说不知道value）
	...
	_prepareInit: function(){
		...
		//IE8下attr()方法不兼容，改用getAttribute
		if (this.element[0].getAttribute("value") && this.options.value) {
			if (this.element[0].getAttribute("value") != this.options.value) {
				this.options.value = this.element[0].getAttribute("value");
			}
		} else if (!this.options.value && this.element[0].getAttribute("value")) {
    		this.options.value = this.element[0].getAttribute("value");
    	}
...
//			this.uiCombo.combo.addClass("coral-hasLabel");
		...
		}).addClass( "coral-combo-wrapper " + this.options.cls + " coral-front" ).hide();
	...
	_bindEvent: function() {
		...
		if ( this.options.disabled ) {
			...
			//和_setDisabled保持一致
			this.uiCombo.combo.addClass( "coral-state-disabled" );
		}
	...
	_doKeyDown: function ( e, searchOnly ) {
		...
		if ( opts.readonly || opts.isLabel || opts.readonlyInput) {
