...
"use strict";	
	...
	$.fn.fmatter.datepicker =function(cval, opts, rwd, act) {
		...
		if(op.disabled===true) {ds = "disabled=\"disabled\"";} else {ds="";}
		if(op.srcDateFormat){
			var format = op.srcDateFormat,
				date = $.coral.parseDate(format, cval , op);
			cval = $.coral.formatDate(format, date, op);
		}
		if (op.valueType == "long") {
			op.restrictConvert = (op.restrictConvert == undefined) ? true : op.restrictConvert;
			cval = $.coral.longToStringDate(cval, op);
		}
		if($.fmatter.isEmpty(cval) || $.fmatter.isUndefined(cval) ) {cval = "";}
		cval=cval+"";
		//cval=cval.toLowerCase();
	...
	$.fn.fmatter.convertCode =function(cval, opts, rwd, act) {
		...
		if(opts.colModel.editoptions && opts.colModel.editoptions.dateFormat && opts.colModel.edittype == "datepicker"){
			var format = opts.colModel.editoptions.dateFormat ? opts.colModel.editoptions.dateFormat:"yyyy-MM-dd";
			var date = $.coral.parseDate(format, cval,opts);
			var cdate = $.coral.formatDate(format, date, opts);
			return cdate;
		}
		var valArr = [],
			has = false,
