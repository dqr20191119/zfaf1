...
$.coral.stringToLongDate = function (value, opts) {
	...
	//包含时分秒时直接new Date("yyyy-MM-dd HH:mm:ss") IE下返回的是Invalid Date，所以分开处理
	value = value.length?value.split(/\D/): value; 
	var d;
		...
		for (iFormat = 0; iFormat < format.length; iFormat++) {
			if (literal) {
				...
				switch (format.charAt(iFormat)) {
					...
					//返回一周中的天数
					case "w":
						output += opts.dayOrders[date.getDay()];
					break;
					default:
						output += format.charAt(iFormat);
				}
...
/*		setDefaults: function(options) {
    ...
    _create: function () {
    	...
    	this.regional[""] = { // 默认地域设置
			...
			dayOrders: ['0','1','2','3','4','5','6'],   //第w天
			weekHeader: '周',//周前缀

			yearSuffix: '年',//年与月之间的分隔文字
			monthSuffix: '月' // 月文本配置
		};
    	$.extend(this.options, this.regional[""], $.fn["datepicker"].defaults);
	...
	_initElement: function(){
		...
		var hidden = $("<input id='"+id+"_srcval' name='"+name+"' type='hidden'>");
...
//		if(options.value){
//			var format = getFormatter(value,options.dateFormat, options);
//			var cdate = $.coral.parseDate(format, value,options);
//			var srcDateFormat = options.srcDateFormat;
//			if (this._checkDateRange(cdate) === 2){
//				value = $.coral.formatDate(srcDateFormat, minDate, options);
//			}
...
//		}
	...
	_processDefaultDate : function(that,defaultDate){
		...
		try{
			defaultDateObj = $.coral.parseDate(opts.srcDateFormat,defaultDate,opts);
		}catch(e){
...
//			}
	...
	_doKeyDown: function(event) {
		...
		opts._keyEvent = true;
		if ( opts.isLabel || opts.readonly || opts.readonlyInput ) { 
			//处理IE Backspace出现网页回退的问题
	...
	_inputBlur : function(e){
		...
		if ( (typeof opts.currentYear != "undefined" || opts.currentYear != null) && !opts.isShow ) {
			...
			var patrn = /[\u4E00-\u9FA5]|[\uFE30-\uFFA0]|[a-zA-Z]|[`~!@#\$%\^\&\*\(\)_\+<>\?"\{\},\.\\\;'\[\]\、\（\）\《\》\——\；\，\。\“\”\<\>\！]|^[-/:]|[-/:]$/gi,
				//dateFormatPatrn: 汉字/字母D(dateFormat的D对应周)/数字/特殊字符
			    dateFormatPatrn = /[\u4E00-\u9FA5]|[\uFE30-\uFFA0]|[D]|[0-9]|[`~!@#\$%\^\&\*\(\)_\+<>\?"\{\},\.\\\;'\[\]\、\（\）\《\》\——\；\，\。\“\”\<\>\！]|^[-/:]$/g,
			    dateFormatResult = opts.dateFormat.match(dateFormatPatrn);
			if(!dateFormatResult) {
				//如果dateFormat没有汉字/数字/特殊字符，但输入框内有汉字/特殊字符，则清空
...
//				opts.dpDiv.find("#dpTimeSecond").val(opts.currentSed||"00");
			...
			this._change(oldValue, value, null);
	...
	show: function(event) {
		...
		if ( !opts.isShow ) {
			...
			(function move () {
				var datepickerComponent = $(input).parent();
...
/*						that._apply(inst,"onFormatError",[date,inst]);TODO: 涉及_apply的以后要重写
	...
	_setDate: function(date, noChange) {
		var clear = !date,dateArr=[],timeArr=[],
		    opts = this.options,
...
//					var opts = this.options;
	...
	_generateHTML: function() {
		var opts = this.options,
			...
			printDate, dRow, tbody, dayoptions, otherMonth, unselectable, unactiveable = true,
			tempDate = new Date(),
			...
			for (col = 0; col < numMonths[1]; col++) {
				...
				for (dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (dow = 0; dow < 7; dow++) { // create date picker days
						...
						if (opts.specialDates.length >0) {
							var specialDay = [], specialYear = [], specialMonth =[], specialDate = [];
							for (var i=0;i<opts.specialDates.length;i++) {
								specialDate = new Date(opts.specialDates[i]);
								if(isNaN(specialDate)){ // IE8不支持"2017-09-20"类格式，只能拆分
									specialDate = opts.specialDates[i].split(/\D/); // 分割日期字串
									specialDate= new Date(Number(specialDate[0]),Number(specialDate[1])-1,Number(specialDate[2]));
								}
								specialYear.push(specialDate.getFullYear());
								specialMonth.push(specialDate.getMonth());
								specialDay.push(specialDate.getFullYear().toString() + (specialDate.getMonth()+1)+specialDate.getDate());
							}
							var printyear = printDate.getFullYear(),
								printmonth = printDate.getMonth(),
								printdate = printDate.getDate(),
								printday = printDate.getFullYear().toString() + (printDate.getMonth()+1)+printDate.getDate();
							if ($.inArray(printday, specialDay) > -1) {
								unactiveable = true;
							} else {
								unactiveable = false;
							}
						...
						tbody += "<td class='" +
							...
							(unactiveable ? " coral-state-selectable ":" " + this._unselectableClass + " coral-state-disabled") + 
